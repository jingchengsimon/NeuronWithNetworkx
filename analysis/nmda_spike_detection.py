...
def save_segment_nmda_spike_rate_npz(model, folder_path):
    """
    ...
    """
    if model.seg_v_array is None:
        return
    meta = model._build_segment_metadata()
    n_seg, n_t, n_stim, n_aff, n_trials = model.seg_v_array.shape
    if n_t < 2:
        return
    dt_s = (model.SIMU_DURATION / 1000.0) / (n_t - 1)
    ...
    np.savez_compressed(
        ...
        simu_duration_ms=np.float64(model.SIMU_DURATION),
