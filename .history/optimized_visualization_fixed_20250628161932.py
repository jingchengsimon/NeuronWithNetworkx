...
def process_epoch_data(exp_path: str, epoch_idx: int, rec_loc: str, attr: str, 
    ...
    # 数据预处理 - 修复numpy数组条件判断
    if v is not None and hasattr(v, 'ndim') and v.ndim == 5:
        v = np.mean(v, axis=2)
        if soma is not None:
    ...
    # 计算基线 - 添加安全检查
    v_base_trace = None
    soma_base_trace = None
    apic_v_base_trace = None
    
    if v is not None and hasattr(v, 'shape') and len(v.shape) >= 4:
        try:
            v_base_trace = v[:,:,0,:].reshape(v.shape[0], v.shape[1], 1, v.shape[-1])
        except:
            v_base_trace = None
    
    if soma is not None and hasattr(soma, 'shape') and len(soma.shape) >= 3:
        try:
            soma_base_trace = soma[:,0,:].reshape(soma.shape[0], 1, soma.shape[-1])
        except:
            soma_base_trace = None
    
    if apic_v is not None and hasattr(apic_v, 'shape') and len(apic_v.shape) >= 3:
        try:
    ...
    if rec_loc == 'dend' and v is not None and v_base_trace is not None:
        try:
            if attr == 'peak':
            ...
            print(f"计算dend EPSP时出错: {e}")
            EPSP_list = []
    
    elif rec_loc == 'soma' and soma is not None and soma_base_trace is not None:
        try:
            if attr == 'peak':
            ...
            print(f"计算soma EPSP时出错: {e}")
            EPSP_list = []
    
    elif rec_loc == 'nexus' and apic_v is not None and apic_v_base_trace is not None:
        try:
            if attr == 'peak':
            ...
            print(f"计算nexus EPSP时出错: {e}")
            EPSP_list = []
    
    # 安全计算最大值
    max_value = 0
    if isinstance(EPSP_list, (list, np.ndarray)) and len(EPSP_list) > 0:
        try:
            max_value = float(np.max(EPSP_list))
        except:
            max_value = 0
    ...
    return {
        'epoch_idx': epoch_idx,
        'EPSP_list': EPSP_list,
        'v': v-v_base_trace if v is not None and v_base_trace is not None else None,
        'soma': soma-soma_base_trace if soma is not None and soma_base_trace is not None else None,
        'apic_v': apic_v-apic_v_base_trace if apic_v is not None and apic_v_base_trace is not None else None,
        'max_value': max_value
    }
...
def nonlinearity_visualization_optimized(exp: str, ax_idx: int, exp_idx: int, fig, ax, 
    ...
    if v is not None and hasattr(v, 'ndim') and v.ndim == 5:
        ...
        if nmda_g is not None:
            nmda_g = np.mean(nmda_g, axis=2)
        if ampa_g is not None:
    ...
    if plot_flag:
        ...
        if rec_loc == 'dend' and v is not None and v_base_trace is not None:
            ...
            try:
                ...
                print(f"绘制nexus数据时出错: {e}")

    return (v-v_base_trace if v is not None and v_base_trace is not None else None, 
            soma-soma_base_trace if soma is not None and soma_base_trace is not None else None, 
            apic_v-apic_v_base_trace if apic_v is not None and apic_v_base_trace is not None else None, 
            EPSP_list)
