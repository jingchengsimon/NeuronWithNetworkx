...
class CellWithNetworkx:
    ...
    def add_inputs(self, folder_path):
        ...
        plt.figure()
        for i in range(len(dend_v_list)):
            plt.plot(time_v, dend_v_list[-i], label=i+1)
        plt.legend()
        plt.xlabel('Time (ms)')
        ...
        x_values = np.linspace(0, len(dend_v_peak_list), len(dend_v_peak_list))
        y_values = x_values + np.min(dend_v_peak_list)
    ...
    def _run_simulation(self, ori_dg, stim_id, stim_index):
        ...
        dend_i_nmda = h.Vector().record(syn_ctr._ref_i_AM)
