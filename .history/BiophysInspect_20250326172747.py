...
def full_nonlinearity_visualization(exp_list, idx_list, data_list, stack_flag=False, num_epochs=10):
    ...
    for i in range(num_figs):
        fig, ax = plt.subplots(num_ax_rows, 1+num_epochs//num_ax_rows, figsize=((num_epochs+1) * 3//num_ax_rows, 4*num_ax_rows), sharey=True)
    ...
    plt.savefig(f'/G/results/simulation/202503_Nonlinearity/{anal_loc}_range{range_idx}_{data_list[0]}_gNMDA{g_NMDA}_{spat_cond}_invitro_variedW_tau90_addNaK.jpg')
    
# create path
if not os.path.exists('/G/results/simulation/202503_Nonlinearity/'):
    os.makedirs('/G/results/simulation/202503_Nonlinearity/')

anal_loc, range_idx, col_idx, num_epochs = 'basal', 0, 0, 1 # range_idx: 0-2, col_idx: 0-5
