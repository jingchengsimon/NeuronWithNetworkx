...
def generate_input_spike_trains_for_simulation(sim_duration_ms, basal_seg_length_um, apical_seg_length_um, min_seg_length_um,
    ...
    ex_spikes_bin_poisson = np.random.poisson(lam=ex_spike_rate_per_seg_per_1ms)
    ex_spikes_bin_poisson = (ex_spikes_bin_poisson > 0).astype(int)  # 只关心有无spike

    spike_count = ex_spikes_bin_poisson.sum()
    avg_firing_rate = spike_count / (totalSimDurationInSec * num_syn)
    print(f"平均firing rate: {avg_firing_rate:.4f} Hz")
