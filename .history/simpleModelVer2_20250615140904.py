...
class CellWithNetworkx:
    ...
    def add_inputs(self, folder_path, simu_condition, input_ratio_basal_apic, bg_exc_channel_type, initW, num_func_group, inh_delay, num_trials):
        ...
        spt_rnd = np.random.RandomState(self.epoch_idx) #np.random.RandomState(42)
        spt_unit_array_list = []
        for num_stim in range(1, self.num_stim + 1):
            spt_unit_array = generate_vecstim(self.rnd, self.unit_ids, num_stim, self.stim_time, self.SIMU_DURATION)
            spt_unit_array_list.append(spt_unit_array)
        
        perm = self.rnd.permutation(self.num_preunit)
...
def build_cell(**params):
    ...
    if basal_channel_type == 'AMPANMDA':                                                                                
        simu_folder = sec_type + '_range' + str(distance_to_root) + '_' + spat_condtion + '_' + simu_condition + '_variedW_tau43_addNaK_singclus_woAP+Ca_aligned_varyinh' # + '_ratio1' + '_exc1.1-1.3' + '_inh4' + '_failprob0.5' + '_funcgroup10'
    elif basal_channel_type == 'AMPA':
            simu_folder = sec_type + '_range' + str(distance_to_root) + '_' + spat_condtion + '_' + simu_condition + '_variedW_tau43_addNaK_singclus_woAP+Ca_aligned_varyinh_AMPA' 
