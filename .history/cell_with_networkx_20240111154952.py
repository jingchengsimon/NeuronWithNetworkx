...
class CellWithNetworkx:
    ...
    def add_inputs(self, folder_path):
        ...
        for i in range(0, len(self.dend_v_list), 4):
            plt.plot(time_v, self.dend_v_list[-(i+1)], label=i+1)
        plt.legend()
        plt.xlabel('Time (ms)')
    ...
    def _add_clustered_inputs(self, spt_unit_list, num_syn_to_get_input):  
        ...
        for j in range(self.num_clusters):
            sec_syn_clustered_df = self.section_synapse_df[(self.section_synapse_df['type'] == 'C' & self.section_synapse_df['cluster_id'] == j]
