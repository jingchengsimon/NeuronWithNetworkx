...
class CellWithNetworkx:
    ...
    def assign_clustered_synapses(self, basal_channel_type, sec_type,
        ...
        for i in range(num_clusters):
            ...
            soma = self.section_synapse_df[(self.section_synapse_df['type'] == 'B') & (self.section_synapse_df['region'] == 'soma')].iloc[0]['section_synapse']
            def recursive_distance_to_soma(sec, soma):
                # Base case: if sec is the soma, return 0 distance
                if sec == soma:
                    return 0
                
                # Check if there is a parent section
                if h.SectionRef(sec).has_parent():
                    parent_sec = h.SectionRef(sec).parent
                    # Add the length of the current section to the recursive call for the parent
                    return sec.L + recursive_distance_to_soma(parent_sec, soma)
                else:
                    # If there is no parent, this is unexpected as we should reach the soma
                    raise ValueError("Section does not reach the soma.")

            # Example usage:
            # Assume `sec` is your initial section and `L5PCtemplate[0].soma[0]` is the soma section
            distance_to_soma = recursive_distance_to_soma(syn_ctr, soma)
            print(f"Distance from the section to soma: {distance_to_soma} µm")

            cluster_distance = h.distance(syn_ctr['segment_synapse']) + (syn_ctr['loc'] - 0.5) * syn_ctr['section_synapse'].L
            ...
            cluster_member_distance = [h.distance(syn_surround_ctr.iloc[idx]['segment_synapse']) + (syn_surround_ctr.iloc[idx]['loc'] - 0.5) 
                         * syn_surround_ctr.iloc[idx]['section_synapse'].L for idx in cluster_member_index]
            ...
            self.section_synapse_df.loc[syn_surround_ctr.iloc[cluster_member_index].index, 'cluster_distance'] = cluster_member_distance
    ...
    def add_inputs(self, folder_path, simu_condition, input_ratio_basal_apic, bg_exc_channel_type, initW, inh_delay, num_trials):
        ...
        self.dend_nmda_g_array = np.zeros((self.num_clusters, num_time_points, self.num_stim, num_aff_fibers, num_trials))
        self.dend_ampa_g_array = np.zeros((self.num_clusters, num_time_points, self.num_stim, num_aff_fibers, num_trials))
