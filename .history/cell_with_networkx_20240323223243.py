...
class CellWithNetworkx:
    def __init__(self, swc_file, bg_syn_freq):
        ...
        self.class_dict = set_graph_order(self.G, self.DiG)
    ...
    def assign_clustered_synapses(self, num_clusters, cluster_radius, dis_to_soma,  
                                  num_stim, num_syn_per_cluster, basal_channel_type,folder_path):
        ...
        dist_list = self.class_dict.get(dis_to_soma, [])
    ...
    def add_inputs(self, folder_path):
        ...
        self.dend_v_array = np.zeros((self.num_clusters, num_time_points, int(self.num_preunit/5), num_trials))
        self.dend_i_array = np.zeros((self.num_clusters, num_time_points, int(self.num_preunit/5), num_trials))
        self.dend_nmda_i_array = np.zeros((self.num_clusters, num_time_points, int(self.num_preunit/5), num_trials))
        self.dend_ampa_i_array = np.zeros((self.num_clusters, num_time_points, int(self.num_preunit/5), num_trials))
        self.soma_v_array = np.zeros((num_time_points, int(self.num_preunit/5), num_trials))
        
        basal_channel_type = 'AMPANMDA'
        
        num_syn_to_get_input = -1
        # num_activated_preunit = -1
        # spt_unit_list = []
        # for _ in range(self.num_clusters):
        #     spt_unit_list_per_cluster = []
        #     for _ in range(self.num_syn_per_cluster):
        #         netstim = h.NetStim()
        #         netstim.number = self.num_stim
        ...
        #         netstim.noise = 0
        #         spt_unit_list_per_cluster.append(netstim)
        #     spt_unit_list.append(spt_unit_list_per_cluster)

        # for num_syn_to_get_input in range(self.num_syn_per_cluster):
        for num_activated_preunit in range(4, self.num_preunit, 5):
            # spt_unit_list_truncated = [spt_unit_list[i][:num_syn_to_get_input+1] for i in range(self.num_clusters)]
            spt_unit_list_truncated = [spt_unit_list[i] for i in range(num_activated_preunit)]
            ...
            for trial_idx in range(num_trials):
                ...
                # Run the simulation
                # self.run_simulation(num_syn_to_get_input, trial_idx)
                self.run_simulation(num_activated_preunit, trial_idx)
        ...
        np.save(os.path.join(folder_path,'dend_ampa_i_array.npy'), self.dend_ampa_i_array)
            ...
            # np.save(os.path.join(folder_path,'soma_v_array_'+basal_channel_type+'.npy'), self.soma_v_array) 
            # np.save(os.path.join(folder_path,'dend_v_array_'+basal_channel_type+'.npy'), self.dend_v_array)
            # np.save(os.path.join(folder_path,'dend_i_array_'+basal_channel_type+'.npy'), self.dend_i_array)
            # np.save(os.path.join(folder_path,'dend_nmda_i_array_'+basal_channel_type+'.npy'), self.dend_nmda_i_array)
            # np.save(os.path.join(folder_path,'dend_ampa_i_array_'+basal_channel_type+'.npy'), self.dend_ampa_i_array)

    def run_simulation(self, num_syn_to_get_input, trial_idx):
        ...
        self.soma_v_array[:, num_syn_to_get_input, trial_idx] = np.array(soma_v)
        for cluster_id in range(self.num_clusters):
            self.dend_v_array[cluster_id, :, num_syn_to_get_input, trial_idx] = np.array(dend_v_list[cluster_id])
            self.dend_i_array[cluster_id, :, num_syn_to_get_input, trial_idx] = np.array(dend_i_list[cluster_id])
            self.dend_nmda_i_array[cluster_id, :, num_syn_to_get_input, trial_idx] = np.array(dend_i_nmda_list[cluster_id])
            self.dend_ampa_i_array[cluster_id, :, num_syn_to_get_input, trial_idx] = np.array(dend_i_ampa_list[cluster_id])
