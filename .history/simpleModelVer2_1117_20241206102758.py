...
class CellWithNetworkx:
    ...
    def assign_clustered_synapses(self, basal_channel_type, sec_type, dis_to_root, 
        ...
        for i in range(self.num_clusters):
            ...
            # Assign the surround as clustered synapse only if more than 1 syn per cluster (disper)
            if num_syn_per_cluster > 1:
                ...
                max_syn_per_clus = 100
                try:
                    dis_mark_from_ctr = np.sort(self.rnd.exponential(cluster_radius, num_syn_per_cluster - 1))
                    max_dis_mark_from_ctr = np.sort(self.rnd.exponential(cluster_radius, max_syn_per_clus - 1))
                except ValueError:
