...
def generate_input_spike_trains_for_simulation(sim_duration_ms, basal_seg_length_um, apical_seg_length_um, min_seg_length_um,
    ...
    plt.plot(np.mean(ex_spike_rate_per_seg_per_1ms[:len(adjusted_basal_length_um)], axis=0), color='b',la
    plt.plot(np.mean(ex_spike_rate_per_seg_per_1ms[len(adjusted_basal_length_um):], axis=0), color='r')
    plt.xlabel('Time (ms)')
