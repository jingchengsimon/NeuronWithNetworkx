    ...
    def assign_clustered_synapses(self, basal_channel_type, sec_type, dis_to_root, 
        ...
        for i in range(self.num_clusters):
            ...
            while True:
                ...
                # rng = np.random.RandomState(self.epoch_idx + loop_count)
                ...
                if num_syn_per_clus > 1:
                    ...
                    # while len(dis_syn_from_ctr) < num_syn_per_clus - 1:
                        ...
                        if list(self.DiG.predecessors(syn_pre_sec_id)) not in ([], [0], [121]):
                            syn_pre_sec_id = self.rng.choice(list(self.DiG.predecessors(syn_pre_sec_id)))
                            try:
...
def build_cell(**params):
    ...
    simu_folder = sec_type + '_range' + str(distance_to_root) + '_' + spat_condtion + '_' + simu_condition + '_variedW_tau90_addNaK_overallbranchtest_woAP+Ca_withclus_aligned' # + '_ratio1' + '_exc1.1-1.3' + '_inh4' + '_failprob0.5' + '_funcgroup10'
