...
class CellWithNetworkx:
    ...
    def assign_clustered_synapses(self, basal_channel_type, sec_type, dis_to_root, 
        ...
        max_num_preunit_per_clus = 32
        max_num_preunit = max_num_preunit_per_clus * num_clusters

        if spat_condition == 'clus':
            # Number of synapses in each cluster is not fixed
            self.pref_ori_dg, max_indices = generate_indices(self.rnd, num_clusters, 
                                                                    num_conn_per_preunit, max_num_preunit)
        elif spat_condition == 'distr':
            # num_pre*num_conn clus with 1 syn per 'cluster'
            num_clusters = max_num_preunit * num_conn_per_preunit
            numbers = np.repeat(np.arange(max_num_preunit), num_conn_per_preunit)
            np.random.shuffle(numbers)
            max_indices = [[num] for num in numbers]
        ...
        for i in range(self.num_clusters):
            ...
            max_num_syn_per_cluster = len(max_indices[i])   
            ...
            try:
                ...
                dis_mark_from_ctr = np.sort(self.rnd.exponential(cluster_radius, max_num_syn_per_cluster - 1))
            except ValueError:
            ...
            max_cluster_member_index = distance_synapse_mark_compare(dis_syn_from_ctr, dis_mark_from_ctr)
            cluster_memeber_index = self.rnd.choice(max_cluster_member_index, num_syn_per_cluster - 1, replace=False)
