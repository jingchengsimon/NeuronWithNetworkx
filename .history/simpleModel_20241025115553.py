...
class CellWithNetworkx:
    ...
    def assign_clustered_synapses(self, basal_channel_type, sec_type,
        ...
        for i in range(num_clusters):
            ...
            def recursive_distance_to_soma(sec, loc, soma, initial=True):
                # Base case: if sec is the soma, return distance to the center of the soma
                if sec == soma:
                    return 0.5 * sec.L
                ...
                if h.SectionRef(sec).has_parent():
                    ...
                    if initial:
                        distance = sec.L * syn_ctr['loc']
                    else:
                        distance = sec.L * loc
                    # Add the length of the current section to the recursive call for the parent
                    return sec.L + recursive_distance_to_soma(parent_sec, soma, initial=False)
                else:
                    # If there is no parent, this is unexpected as we should reach the soma
                    raise ValueError("Section does not reach the soma.")

            cluster_distance = recursive_distance_to_soma(syn_ctr['section_synapse'], syn_ctr['loc'], soma, initial=True)
