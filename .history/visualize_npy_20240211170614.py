...
for target_values in target_values_list:
    ...
    if result_folders:
        for folder_path in result_folders:
            ...
            dend_v_peak = np.mean(np.max(np.array(dend_v_array)[:,20000:24000,:,:]),axis=2)
            plt.figure()
            for cluster_index in range(dend_v_array.shape[0]):
                for i in range(0, dend_v_array.shape[2], 4):
                    plt.plot(dend_v_array[cluster_index, :, i], label=f"Cluster {cluster_index + 1} synapse {i + 1}")
            # 对 num_trials 维度取平均
