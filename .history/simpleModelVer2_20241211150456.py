            ...
            while exceed_flag == False:
                ...
                if num_syn_per_clus > 1:
                    ...
                    # while len(dis_syn_from_ctr) < num_syn_per_clus - 1:
                        ...
                        suc_exceed_flag = not all(sec_id in np.unique(sec_syn_bg_exc_ordered_df['section_id_synapse']) for sec_id in list(self.DiG.successors(syn_suc_sec_id)))
                        pre_exceed_flag = not all(sec_id in np.unique(sec_syn_bg_exc_ordered_df['section_id_synapse']) for sec_id in list(self.DiG.predecessors(syn_pre_sec_id)))
                        exceed_flag = suc_exceed_flag and pre_exceed_flag and (len(dis_syn_from_ctr) < num_syn_per_clus - 1)
