...
class CellWithNetworkx:
    def __init__(self, swc_file, bg_syn_freq):
        ...
        self.section_df, self.DiG, self.sp = create_graph(self.all_sections, self.section_df)
        # assign the order for each section
        self.class_dict_soma, self.class_dict_tuft = set_graph_order(self.G, self.DiG, self.root_tuft)
    ...
    def assign_clustered_synapses(self, basal_channel_type, sec_type,
        ...
        if sec_type == 'basal':
            dist_list = self.class_dict_soma.get(dis_to_soma, [])
        elif sec_type == 'tuft':
            dist_list = self.class_dict_tuft.get(dis_to_soma, []) 
    ...
    def add_inputs(self, folder_path, num_trials):
        ...
        num_activated_preunit_list = [1,5,10,25,50,100] # currently don't exceed 100
