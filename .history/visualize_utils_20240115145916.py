...
def visualize_synapses(sectionfolder_path, title='Synapses'):
        s = h.PlotShape(False)
        recursive_plot(s, section_synapse_df['segment_synapse'].values)
        plt.title(title)
...
def recursive_plot(s, seg_list, index=0):
    markers = {
        'A': 'or',  # 红色圆形
        'B': 'xb',  # 蓝色叉形
        'C': 'sg',  # 绿色方形
    ...
    if index == 0:
        ...
        type_array = section_synapse_df['type'].values
