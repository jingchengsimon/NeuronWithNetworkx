...
class CellWithNetworkx:
    ...
    def add_inputs(self, folder_path, bg_exc_channel_type, initW, inh_delay, num_trials):
        ...
        exc_syn_on_clustered_sec = self.section_synapse_df[(self.section_synapse_df['section_synapse'].isin(clustered_sec)) & 
                                                           (self.section_synapse_df['type'] == 'A')]['sy']
        self.bg_exc_i_nmda_array = np.zeros((exc_syn_on_clustered_sec.shape[0], num_time_points, num_aff_fibers, num_trials))
        self.bg_exc_i_ampa_array = np.zeros((exc_syn_on_clustered_sec.shape[0], num_time_points, num_aff_fibers, num_trials))
