...
class CellWithNetworkx:
    ...
    def assign_clustered_synapses(self, basal_channel_type, sec_type, dis_to_root, 
        ...
        if sec_type == 'basal':
            num_syn_thres = [3300 + i * 3300 for i in range(2)]
        ...
        # num_syn_per_clus = num_syn_per_clus * num_clusters
        ...
        if spat_condition == 'clus':
            ...
            self.unit_ids = np.arange(num_syn_per_clus)

            # num_pre*num_conn clus with 1 syn per 'cluster'
            numbers = np.repeat(np.arange(num_syn_per_clus), num_conn_per_preunit)
            np.random.shuffle(numbers)
            indices = [[num] for num in numbers]

            num_clusters = num_syn_per_clus * num_conn_per_preunit
        ...
        with open(file_path, 'w') as f:
            for i, index_list in enumerate(indices):
                f.write(f"Cluster_id: {i}, num_syn_per_cluss: {len(index_list)}, Preunit_ids: {index_list}\n")
        ...
        self.num_syn_per_clus = num_syn_per_clus
    ...
    def add_inputs(self, folder_path, simu_condition, input_ratio_basal_apic, bg_exc_channel_type, initW, inh_delay, num_trials):
        ...
        self.num_activated_preunit_list = range(0, self.num_syn_per_clus + 2, 2)
...
def build_cell(**params):
    ...
    folder_path = '/G/results/simulation/' + time_tag + '/' + folder_tag + '/' + str(epoch)
    ...
    simulation_params = {
        ...
        'number of preunit': num_syn_per_clus,
...
def run_processes(parameters_list):
    ...
    for params in parameters_list:
        for epoch in range(1, 6): # run different assignments with same parameters for 5 times
