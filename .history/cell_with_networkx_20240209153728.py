...
class CellWithNetworkx:
    ...
    def add_inputs(self, folder_path):
        ...
        for num_syn_to_get_input in range(self.num_syn_per_cluster):
            ...
            # create an ndarray to store the voltage of each t
            volatge_array = np.zeros((len(self.ori_dg_list), len(self.unit_ids), self.num_syn_per_cluster))

            # For analysing the supra-linearity by NMDA
            time_v, dend_v, soma_v = self.run_simulation(ori_dg, stim_id, stim_index, num_syn_to_get_input, folder_path)
