...
def nonlinearity_visualization(exp, k, fig, ax, data_type, stack_flag, exp_idx, alpha=1):
    ...
    v_baseline = np.average(v[:, (t-200)*40:(t-100)*40, :, 0]) # [self.num_clusters_sampled, num_time_points, self.num_stim, num_aff_fibers, num_trials]
    soma_baseline = np.average(soma[(t-200)*40:(t-100)*40, :, 0])
    apic_v_baseline = np.average(apic_v[(t-200)*40:(t-100)*40, :, 0])
...
def full_nonlinearity_visualization(exp_list, idx_list, data_list, stack_flag=False, num_epochs=10):
    ...
    plt.savefig(f'/G/results/simulation/202503_Nonlinearity/{anal_loc}_range{range_idx}_{data_list[0]}_gNMDA{g_NMDA}_{spat_cond}_invivo_variedW_tau90_addNaK.jpg')
...
for anal_loc in ['basal']:
    for spat_cond in ['clus', 'distr']:
        for range_idx in range(3):
            for attr in ['dend', 'nexus']:
                full_nonlinearity_visualization([anal_loc+f'_range{range_idx}_{spat_cond}_invivo_variedW_tau90_addNaK_overallbranchtest_varynumsyn_step2'] * iter_times,
