...
def add_background_exc_inputs(section_synapse_df, syn_param_exc, DURATION, FREQ_EXC, input_ratio_basal_apic,bg_exc_channel_type, initW, lock):
    ...
    def process_section(i):
        ...
        if section['region'] == 'basal':
            ...
            counts = np.random.poisson(FREQ_EXC/3000 * pink_noise)
...
def add_background_inh_inputs(section_synapse_df, syn_param_inh, DURATION, FREQ_INH, num_syn_inh_list, inh_delay, lock):  
    ...
    spike_counts_apic_inh = np.random.poisson(lambda_array/3, size=(num_syn_apic_inh, DURATION))
