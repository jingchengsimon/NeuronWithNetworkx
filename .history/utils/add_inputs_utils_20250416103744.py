...
def add_background_inh_inputs(section_synapse_df, syn_param_inh, DURATION, FREQ_INH, 
    ...
    spike_time_clus = section_synapse_df_clus[(section_synapse_df_clus['type'].isin(exc_types)) &
                                                (section_synapse_df_clus['cluster_flag'] == 1)]['spike_train']
    total_spikes = np.zeros(DURATION)  # 初始化总spike数数组

    for spike_times_vec_bg in spike_times_bg:
