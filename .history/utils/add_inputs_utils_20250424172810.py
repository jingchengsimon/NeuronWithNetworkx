...
def add_background_inh_inputs(section_synapse_df, syn_param_inh, DURATION, FREQ_INH, 
    ...
    if all(len(spike)==0 for spike in sec_syn_bg_inh_df['spike_train_bg']):
        ...
        spike_trains_inh_bg = rnd.poisson(lambda_array_bg/1000, size=(int(num_syn_bg_inh/1), DURATION))
    else:
    ...
    spike_trains_inh_sync = rnd.poisson(lambda_array_sync/1000, size=(int(num_syn_bg_inh/1), DURATION))
    ...
    for j in range(num_clusters):
        ...
        for i in range(num_syn_clustered):
            ...
            try:
                ...
                spt_unit_nobg = spt_unit_array[spt_unit_array['pre_unit_id'] == section['pre_unit_id']]['spt_unit'][0]
            except IndexError:
                spt_unit_nobg = np.array([])

            # Invivo background
            if len(section['spike_train_bg']) > 0:
                spike_train_bg = section['spike_train_bg'][0]
                spt_unit = np.unique(np.concatenate((spike_train_bg, spt_unit_nobg))) # each time point only once
            ...
            section_synapse_df.at[section.name, 'netstim'] = netstim
            section_synapse_df.at[section.name, 'spike_train'].append(list(spt_unit_nobg))
            section_synapse_df.at[section.name, 'netcon'] = netcon
