...
def create_directed_graph(all_sections, section_df):
    ...
    for i, section_segment_list in enumerate(all_sections):
        ...
        # Define branch index
        if parent_id ==  0 & section not in branch_init_sec_list:
            branch_init_sec_list.append(section)
            branch_idx = branch_init_sec_list.index(section)
        elif parent_id != 0 & section_df[section_df['section_name'] == parent.psection()['name']]['branch_idx'].values[0] is:
        # create data
