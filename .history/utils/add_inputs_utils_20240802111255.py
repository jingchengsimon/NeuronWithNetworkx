...
def add_background_exc_inputs(section_synapse_df, syn_param_exc, DURATION, FREQ_EXC, bg_exc_channel_type, initW, lock):
    ...
    def process_section(i):
        ...
        # counts = np.random.poisson(FREQ_EXC, DURATION)
        # spike_times_int_exc = np.repeat(np.arange(DURATION), counts)
        # spike_train = spike_times_int_exc + np.random.rand(np.sum(counts))

        # netstim = h.VecStim()
        # spike_train_vec = h.Vector(spike_train)    
        # netstim.play(spike_train_vec)
...
def add_background_inh_inputs(section_synapse_df, syn_param_inh, DURATION, FREQ_INH, num_syn_inh_list, inh_delay, lock):  
    ...
    def process_section(i):
        ...
        # spike_train = np.where(counts >= 1)[0] + np.random.rand(np.sum(counts >= 1))
        spike_times_int_inh = np.repeat(np.arange(DURATION), counts)
        spike_train = spike_times_int_inh + len(spike_times_int_inh)np.random.rand(np.sum(counts)) # np.sum(counts) == len(spike_times_int_inh)
