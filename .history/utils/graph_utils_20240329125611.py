...
def set_graph_order(G, DiG, apical_tuft_start_point_idx):
    ...
    for node in order_dict.keys():
        order = nx.shortest_path_length(G, source=apical_tuft_start_point_idx, target=node)
        if order not in class_apical_tuft_dict:
            class_apical_tuft_dict[order] = []
        class_apical_tuft_dict[order].append(node)
