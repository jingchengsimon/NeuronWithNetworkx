...
for node, order in shortest_paths.items():
    if order not in shortest_paths:
        shortest_paths[order] = []
    shortest_paths[order].append(node)


# 统计每个节点的最短路径中分岔点的个数，并设为 order
order = {}
for node, distance in shortest_paths.items():
    forks_in_path = sum(1 for n in shortest_paths[node][1:] if n in fork_points)
