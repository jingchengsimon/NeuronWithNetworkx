...
def add_background_exc_inputs(section_synapse_df, syn_param_exc, DURATION, FREQ_EXC, 
    ...
    with ThreadPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
        executor.map(process_generate_basal_spike_train, range(num_preunit_exc_basal))

    with ThreadPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
        executor.map(process_generate_apical_spike_train, range(num_preunit_exc_apical))
    ...
    def process_section(i):
        ...
        if section['region'] == 'basal':
            spike_train = np.random.choice(basal_spike_trains)
        elif section['region'] == 'apical':
            spike_train = np.random.choice(apical_spike_trains)
...
def add_background_inh_inputs(section_synapse_df, syn_param_inh, DURATION, FREQ_INH, 
    ...
    spike_counts_basal_inh = np.random.poisson(lambda_array, size=(int(num_syn_basal_inh/12), DURATION))
    spike_counts_apic_inh = np.random.poisson(lambda_array/input_ratio_basal_apic, size=(int(num_syn_apic_inh/12), DURATION))
