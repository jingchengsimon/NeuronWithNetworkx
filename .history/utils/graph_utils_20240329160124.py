...
def set_graph_order(G, DiG, tuft_root):
    ...
    ## distance to the start point of apical tuft
    class_apical_tuft_dict = {}
    ## select the child point of the start point of apical tuft
    # for node in order_dict.keys():
    #     forks_in_path = sum(1 for n in nx.shortest_path(G, source=0)[node][1:-1] if n in fork_points)
    #     order = forks_in_path
    #     if order not in class_apical_tuft
    # tuft_root 
    # max_order = max(class_dict)
    # for i in range(max_order + 1):
        # print(f"Class {i}: {class_dict.get(i, [])}")

    class_dict_soma = calculate_out_degree(G, DiG, 0)
    sub_G_tuft = get_subgraph(G, tuft_root)
    sub_DiG_tuft = get_subgraph(DiG, tuft_root)
    class_dict_tuft = calculate_out_degree(sub_G_tuft, sub_DiG_tuft, tuft_root)
    return class_dict, class_dict_soma, class_dict_tuft
...
def calculate_out_degree(G, DiG, root_node=0):
