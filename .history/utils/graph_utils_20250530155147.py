...
def create_directed_graph(all_sections, all_segments, section_df):
    ...
    segment_DiG_array = nx.to_numpy_array(segment_DiG, nodelist=sorted(segment_DiG.nodes()))  # 可选指定 node 顺序
    segment_DiG_json_matrix = json.dumps(segment_DiG_array.tolist())  # 转换为 JSON 格式的字符串

    print(segment_DiG_json_matrix)
