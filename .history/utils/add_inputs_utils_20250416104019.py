...
def add_background_inh_inputs(section_synapse_df, syn_param_inh, DURATION, FREQ_INH, 
    ...
    for spike_times_bg_list, spike_times_clus_list in zip(spike_times_bg, spike_time_clus):
        spike_times_vec = spike_times_bg_list[0]  # 'spike train' is a list of lists (spike_trains)
        if spike_times_clus_list is not None and len(spike_times_clus_list) > 0:
            spike_times_vec_clus = spike_times_clus_list[-1]
            spike_times_vec = np.concatenate((spike_times_vec, spike_times_vec_clus))
