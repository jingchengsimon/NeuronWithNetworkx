...
def add_background_exc_inputs(section_synapse_df, syn_param_exc, DURATION, FREQ_EXC, 
    ...
    syn_w_distr = np.random.lognormal(mean=initW, sigma=1, size=50000)
    np.random(int(time.time())) # Reset the random number generator
    ...
    def process_section(i):
        ...
        if section['synapse'] is None:
            ...
            if bg_exc_channel_type == 'Exp2Syn':
                ...
                print(f'initW_distr: {initW_distr}')
...
def add_clustered_inputs(section_synapse_df, 
    ...
    # Generate log-normal distribution
    np.random.seed(42)  # For reproducibility
    syn_w_distr = np.random.lognormal(mean=initW, sigma=1, size=10000)
    np.random(int(time.time())) # Reset the random number generator
