...
def plot_graph(G):
    ...
    # Define colors based on node indices
    node_colors = []
    for node in G.nodes:
        if node == 0:
            ...
            node_colors.append('#FFD1DC')  # Light pink color for descendants of 121
        else:
            node_colors.append('lightgray')  # Default color for nodes outside specified ranges

    # Set positions and draw nodes with specified colors
    pos = graphviz_layout(G, prog="dot")
    nx.draw(G, pos, with_labels=False, node_size=70, node_color=node_colors)
