...
def create_directed_graph(all_sections, all_segments, section_df):
    ...
    for section_id, segment_list in enumerate(all_sections):
        ...
        for seg_idx in range(num_segments):
            ...
            segment_id_map[segment_index] = segment_global_id
            segment_global_id += 1
    ...
    for section_id, section_segment_list in enumerate(all_sections):
        ...
        parent_seg_end = all_segments.index(all_segments[section_segment_list[-1]])  # last segment of parent section
        child_seg_start = all_segments.index(section_segment_list[0]) 
        (section_id, 0)
    ...
    plt.title("Segment-Level DiGraph")
    plt.show()
    ...
    # Save the PhyloXML tree to an XML file
    with open("L5_morphology_segment.xml", "w") as xml_file:
        Phylo.write([phylo_tree], xml_file, "phyloxml")
