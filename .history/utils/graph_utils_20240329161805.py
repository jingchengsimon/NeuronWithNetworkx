...
def create_dgraph(all_sections, section_df):
...
def set_graph_order(DiG, root_tuft):
    ...
    class_dict_soma = calculate_out_degree(DiG, 0)

    DiG_tuft = get_subgraph(DiG, root_tuft)
    class_dict_tuft = calculate_out_degree(DiG_tuft, root_tuft)
...
def calculate_out_degree(DiG, root_node=0):
    order_dict = nx.single_source_shortest_path_length(DiG, root_node)
    ...
    class_dict = {}
    for node in order_dict.keys():
        forks_in_path = sum(1 for n in nx.shortest_path(DiG, source=root_node)[node][1:-1] if n in fork_points)
        order = forks_in_path
        if order not in class_dict:
            class_dict[order] = []
        class_dict[order].append(node)
