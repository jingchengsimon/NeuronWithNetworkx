...
def generate_vecstim(rnd, pre_unit_ids, num_stim, stim_time, DURATION):
    ...
    for pre_unit_id in pre_unit_ids:
        ...
        # Use rnd   # to generate random spike times around stim_time
        spt_unit = np.floor(rnd.normal(loc=stim_time, scale=5, size=num_stim)).astype(int)  # Generate random spike times around stim_time with a small standard deviation
