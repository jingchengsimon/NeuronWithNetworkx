...
def add_background_exc_inputs(section_synapse_df, syn_param_exc, DURATION, FREQ_EXC, 
    ...
    initW_distr_list = loc_rnd.choice(len(syn_w_distr), size=num_syn_bg_exc, replace=True)

    e_syn, tau1, tau2 = syn_param_exc

    def process_section(i):
        
        spk_rnd = np.random.default_rng(spk_epoch_idx + i)  # Create a new random state
    ...
    for j in range(num_clusters):
        ...
        for i in range(num_syn_clustered):
            ...
            if section['synapse'] is None:
                ...
                loc_rnd = np.random.default_rng(epoch_idx) # Reset random state before each choise process
