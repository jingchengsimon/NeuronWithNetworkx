...
def add_background_inh_inputs(section_synapse_df, syn_param_inh, DURATION, FREQ_INH, 
                              ...
                              spat_condition, section_synapse_df_clus, num_activated_preunit, lock):  
    ...
    if all(len(spike)==0 for spike in sec_syn_bg_inh_df['spike_train_bg']):
        ...
        # spike_trains_inh_bg = sec_syn_bg_inh_df['spike_train_bg'][0] 
    ...
    spike_trains_inh = np.array(spike_counts_inh_bg, dtype=object)  # Initialize with background spike trains
# each time point only once
    ...
    def process_section(i):
        ...
        if spat_condition == 'clus':
            ...
            spike_train_bg = ast.literal_eval(section_clus['spike_train_bg'])[num_activated_preunit] # the corresponding spike train with the identical number of synchronous inputs
