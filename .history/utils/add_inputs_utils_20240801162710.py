...
def add_background_inh_inputs(section_synapse_df, syn_param_inh, DURATION, FREQ_INH, num_syn_inh_list, inh_delay, lock):  
    ...
    def process_section(i):
        ...
        # A better to way to generate spike train is to repeat the time indices of each spike instead to 
        # just record the time point with spikes more than 1 (since we should repeat those time points with spikes mor than 2)
        spike_times_int_inh = np.repeat(np.arange(DURATION), counts)
        spike_trains = spike_times_int_inh + np.random.rand(np.sum(counts)) # np.sum(counts) == len(spike_times_int_inh)
