...
def create_directed_graph(all_sections, section_df):
    ...
    segment_id_map = {}  # optional, to map (section_id, seg_idx) to global id
    ...
    for section_id, segment_list in enumerate(all_sections):
        ...
        for seg_idx in range(num_segments):
            segment_name = (section_id, seg_idx)
            segment_DiG.add_node(segment_name)

            # Optional: assign global ID
            segment_id_map[segment_name] = segment_global_id
            segment_global_id += 1

            # Add intra-section edge
            if seg_idx > 0:
                prev_seg = (section_id, seg_idx - 1)
                segment_DiG.add_edge(prev_seg, segment_name)
    ...
    for section_id, section_segment_list in enumerate(all_sections):
        ...
        parent_seg_end = (parent_id, len(all_sections[parent_id]) - 1)
        child_seg_start = (section_id, 0)
    ...
    pos = nx.spring_layout(segment_DiG)
    nx.draw(segment_DiG, pos, with_labels=True, node_size=300, font_size=8)

    root = 0
    root_clade = nx_to_clade(segment_DiG, root)
    phylo_tree = Phylogeny.from_clade(root_clade, rooted=True)

    # Save the PhyloXML tree to an XML file
    with open("segment_L5_morphology.xml", "w") as xml_file:
        Phylo.write([phylo_tree], xml_file, "phyloxml")
