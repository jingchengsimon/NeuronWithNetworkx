...
def visualize_summary_simulation(time_v, dend_v_list, dend_v_peak_list, soma_v_list, soma_v_peak_list, folder_path):
    ...
    plt.figure()
    plt.plot(range(1,len(dend_v_peak_list)), dend_v_peak_list, 'r', label='Dendritic peak volatge')
    plt.plot(range(1,len(soma_v_peak_list)), soma_v_peak_list, 'b', label='Soma peak volatge')
    plt.legend()
