...
# Process background and synchronous spike trains separately
for spike_times_bg_list, spike_times_sync_list in zip(spike_times_bg_array, spike_times_sync_array):
    # Background spikes
    if spike_times_bg_list and len(spike_times_bg_list[0]) > 0:
        spike_times_bg = np.floor(spike_times_bg_list[0]).astype(int)
        spike_times_bg = spike_times_bg[spike_times_bg < DURATION]
        total_spikes_bg[spike_times_bg] += 1

    # Synchronous spikes (typically the last element in the list)
...
spike_counts_basal_inh = (
    rnd.poisson(lambda_array_bg / 1000, size=(int(num_syn_basal_inh / 2), DURATION)) +
    rnd.poisson(lambda_array_sync / 1000, size=(int(num_syn_basal_inh / 2), DURATION))
)

spike_counts_apic_inh = (
    rnd.poisson(lambda_array_bg / (input_ratio_basal_apic * 1000), size=(int(num_syn_apic_inh / 2), DURATION)) +
    rnd.poisson(lambda_array_sync / (input_ratio_basal_apic * 1000), size=(int(num_syn_apic_inh / 2), DURATION))
)

spike_counts_soma_inh = (
    rnd.poisson(lambda_array_bg / 1000, size=(int(num_syn_soma_inh / 2), DURATION)) +
    rnd.poisson(lambda_array_sync / 1000, size=(int(num_syn_soma_inh / 2), DURATION))
)
