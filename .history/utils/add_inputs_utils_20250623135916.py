...
def add_background_exc_inputs(section_synapse_df, syn_param_exc, DURATION, FREQ_EXC, 
    ...
    def process_section(i):
        ...
        spk_rnd = np.random.default_rng(epoch_idx + i)  # Create a new random state
        ...
        if spat_condition == 'clus':
            ...
            pink_noise = pink_noise_array[spk_rnd.integers(num_func_group)] #pink_noise_array[rnd.randint(num_func_group)]
...
def add_background_inh_inputs(section_synapse_df, syn_param_inh, DURATION, FREQ_INH, 
                              input_ratio_basal_apic, num_syn_inh_list, inh_delay, epoch_idx, 
                              spat_condition, section_synapse_df_clus, num_activated_preunit_idx, lock):  
    
    # np.random.seed(42)  # For reproducibility
    spk_rnd = np.random.default_rng(epoch_idx)  # Create a new random state
...
def add_clustered_inputs(section_synapse_df, num_clusters, basal_channel_type, initW, spt_unit_array, 
                         rnd):  
