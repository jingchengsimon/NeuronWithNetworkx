...
def add_background_exc_inputs(section_synapse_df, syn_param_exc, DURATION, FREQ_EXC, 
    ...
    def process_section(i):
        ...
        # Clustered synapses already have spike trains (synchronous input around 500ms)
        if section['cluster_flag'] == 1:
            spike_train_clus = section['spike_train'][0]
            spike_train = np.unique(np.concatenate((spike_train, spike_train_clus))) # each time point only once
