...
def add_background_inh_inputs(section_synapse_df, syn_param_inh, DURATION, FREQ_INH, 
    ...
    spike_counts_soma_inh = np.random.poisson(lambda_array, size=(int(num_syn_soma_inh/1), DURATION))
    ...
    for j in range(num_clusters):
        ...
        for i in range(num_syn_clustered):
            ...
            # or no preunit is connected to this section
                ...
                # spt_unit = np.array([])
                section_synapse_df.at[section.name, 'type'] = 'A'
                section_synapse_df.at[section.name, 'cluster_center_flag'] = -1
                section_synapse_df.at[section.name, 'cluster_id'] = -1
                continue # Skip the rest and save time
