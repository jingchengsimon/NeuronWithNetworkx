...
def add_background_inh_inputs(section_synapse_df, syn_param_inh, DURATION, FREQ_INH, 
    ...
    spike_times_bg = section_synapse_df[section_synapse_df['type'].isin(exc_types)]['spike_train_bg'] 
    spike_time_clus = section_synapse_df_clus[(section_synapse_df_clus['type'].isin(exc_types))]['spike_train']
    total_spikes = np.zeros(DURATION)  # 初始化总spike数数组

    for spike_times_vec_bg, spike_times_vec_clus in zip(spike_times_bg, spike_time_clus):
        spike_times_vec = spike_times_vec_bg[0]  # 'spike train' is a list of lists (spike_trains)
        if spike_times_vec_clus is not None and len(spike_times_vec_clus) > 0:
            spike_times_vec_clus = ast.literal_eval(spike_times_vec_clus)[0]
