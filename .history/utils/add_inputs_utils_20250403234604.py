...
def add_background_inh_inputs(section_synapse_df, syn_param_inh, DURATION, FREQ_INH, 
    ...
    spike_times = section_synapse_df[section_synapse_df['type'].isin(exc_types)]['spike_train'] # 'spike train' is a list of lists (spike_trains)
    total_spikes = np.zeros(DURATION)  # 初始化总spike数数组

    for spike_times_vec in spike_times:
        spike_times_vec = spike_times_vec[0]  # 取出第一个元素
