...
def visualize_synapses(section_synapse_df, folder_path, title='Synapses'):
        s = h.PlotShape(False)
        clus_sec_syn_df = section_synapse_df[section_synapse_df['cluster_flag'] == True]
        recursive_plot(s, clus_sec_syn_df['segment_synapse'].values, clus_sec_syn_df['type'].values)
        plt.title(title)
...
def recursive_plot(s, seg_list, type_array, index=0):
    ...
    if index == 0:
        return recursive_plot(s.plot(plt), seg_list, type_array, index+1)
        ...
        return recursive_plot(s.mark(seg_list[index - 1], marker), seg_list, type_array, index+1)
...
def visualize_morpho(section_synapse_df, soma_v, seg_v, folder_path):
    # Choose the time point with the maximum soma voltage
    # Find the index that soma_v first cross 0
    max_soma_idx = np.argmax(np.array(soma_v)[100*40:])+100*40 - 4 # exclude the initial period 
    ...
    # fig1.write_html(folder_path + f"/morpho_{max_soma_idx}.html")
    # fig2.write_html(folder_path + f"/morpho_syn_{max_soma_idx}.html")

def visualization(time_array, series_dict, folder_path, title='trace', ylim=None, show=False):
    """
    A minimal plot helper to visualize multiple 1D traces on a single figure.

    Parameters
    ----------
    time_array : array-like
        1D time axis (e.g., in ms).
    series_dict : dict[str, array-like]
        Mapping from label to 1D data array with the same length as time_array.
        Example: { 'soma_v': soma_trace, 'dend_v': dend_trace }.
    folder_path : str
        Directory to save the figure. Will be created if not present.
    title : str, optional
        Figure title and filename stem.
    ylim : tuple[float, float] | None, optional
        y-axis limits, e.g., (-90, 40). If None, auto-scale.
    show : bool, optional
        Whether to display the figure with plt.show(). Default False.
    """
    if not isinstance(series_dict, dict) or len(series_dict) == 0:
        return
    ...
    for label, data in series_dict.items():
        if data is None:
            continue
        try:
            plt.plot(time_array, data, label=label)
        except ValueError:
