...
def add_background_exc_inputs(section_synapse_df, syn_param_exc, DURATION, FREQ_EXC, 
                              ...
                              epoch_idx, spk_epoch_idx, spat_condition, section_synapse_df_clus, lock):
    ...
    def process_section(i):    
        ...
        if spat_condition == 'clus':
            ...
            pink_noise = pink_noise_array[spk_rnd.integers(num_func_group)] 
...
def add_background_inh_inputs(section_synapse_df, syn_param_inh, DURATION, FREQ_INH, 
    ...
    def convert_counts_to_spike_trains(spike_counts_inh, dropout_p=0.5):
        ...
        for counts in spike_counts_inh:
            ...
            mask = spk_rnd.choice([True, False], size=spike_train.shape, p=[1 - dropout_p, dropout_p])
            spike_trains.append(spike_train[mask])
        return spike_trains
    ...
    # num_syn_inh = num_syn_basal_inh + num_syn_apic_inh + num_syn_soma_inh

    sec_syn_bg_inh_df = section_synapse_df[section_synapse_df['type'] == 'B']
    num_syn_bg_inh = len(sec_syn_bg_inh_df)

    sec_syn_bg_inh_df_clus = section_synapse_df_clus[section_synapse_df_clus['type'] == 'B']


    spike_times_bg_array = section_synapse_df[section_synapse_df['type'].isin(exc_types)]['spike_train_bg'] 
    spike_times_sync_array = section_synapse_df[(section_synapse_df['type'].isin(exc_types)) &
                                                (section_synapse_df['cluster_flag'] == 1)]['spike_train']
    ...
    spike_counts = spk_rnd.poisson(lambda_array/1000, size=(int(num_syn_bg_inh/1), DURATION))
    spike_trains = convert_counts_to_spike_trains(spike_counts)
    ...
    if all(len(spike)==0 for spike in sec_syn_bg_inh_df['spike_train_bg']):
        ...
        spike_counts_inh_bg = spk_rnd.poisson(lambda_array_bg/1000, size=(int(num_syn_bg_inh/1), DURATION))
        spike_trains_inh_bg = convert_counts_to_spike_trains(spike_counts_inh_bg)
    ...
    # Compute separate lambda arrays for background and synchronous spikes
    # lambda_array_sync = FREQ_INH * total_spikes_sync / np.mean(total_spikes_sync[total_spikes_sync>0]) if np.mean(total_spikes_sync[total_spikes_sync>0]) > 0 else np.zeros(DURATION)
    lambda_array_sync = FREQ_INH * total_spikes_sync / np.mean(total_spikes_bg) if np.mean(total_spikes_bg) > 0 else np.zeros(DURATION)
    spike_counts_inh_sync = spk_rnd.poisson(lambda_array_sync/1000, size=(int(num_syn_bg_inh/1), DURATION))
    spike_trains_inh_sync = convert_counts_to_spike_trains(spike_counts_inh_sync)

    ## each time point only once
    spike_trains_inh = np.array([np.unique(np.concatenate((bg, sync))) 
                                 for bg, sync in zip(spike_trains_inh_bg, spike_trains_inh_sync)], dtype=object)
    # spike_trains_inh = np.array(spike_trains_inh_bg, dtype=object)  
    
    def process_section(i):
        
        section = sec_syn_bg_inh_df.iloc[i]
        section_clus = sec_syn_bg_inh_df_clus.iloc[i]

        if section['synapse'] is None:
            synapse = h.Exp2Syn(sec_syn_bg_inh_df.iloc[i]['segment_synapse'])
            synapse.e, synapse.tau1, synapse.tau2 = e_syn, tau1, tau2
        else:
            synapse = section['synapse']
...
def add_clustered_inputs(section_synapse_df, num_clusters, basal_channel_type, initW, spt_unit_array, epoch_idx):  
    ...
    num_syn_clustered_per_cluster = len(section_synapse_df[
        ...
        (section_synapse_df['cluster_id'] == 0)  # 假设所有 cluster 的数量一样
    ])

    num_syn_clus_exc = num_clusters * num_syn_clustered_per_cluster
    initW_distr_array = loc_rnd.choice(syn_w_distr, size=num_syn_clus_exc)
    ...
    for j in range(num_clusters):
        ...
        for i in range(num_syn_clustered):
            ...
            if section['synapse'] is None:
                ...
                # initW_distr = loc_rnd.choice(syn_w_distr, 1)[0]  # variedW
                # glob_idx = j * num_syn_clustered_per_cluster + i
                initW_distr =  initW_distr_array[glob_idx]
                # initW_distr =  0.0100 #initW # fixedW
                syn_params['initW'] = initW_distr
                synapse = AMPANMDA(syn_params, section['loc'], section['section_synapse'], ccccccccccccccccccccccccccccccccccccccccccccccccccccbasal_channel_type)
            else:
                synapse = section['synapse']

            try:
                # spt_unit = spt_unit_list[section['pre_unit_id']]
                spt_unit_nonbg = spt_unit_array[spt_unit_array['pre_unit_id'] == ccccccccccccccccccccccccccccccccccccccccccccccccccc]['spt_unit'][0]
            except IndexError:
            ...
            if len(section['spike_train_bg']) > 0:
                ...
                spt_unit = np.unique(np.concatenate((spike_train_bg, spt_unit_nonbg))) # each time point only once
                # section_synapse_df.at[section.name, 'spike_train'] = []
            else:
                spt_unit = spt_unit_nonbg
