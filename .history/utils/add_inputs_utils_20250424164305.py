...
def add_background_inh_inputs(section_synapse_df, syn_param_inh, DURATION, FREQ_INH, 
    ...
    if sec_syn_bg_inh_df['spike_train_bg'] is None:
        ...
        spike_trains_inh_bg = sec_syn_bg_inh_df['spike_train_bg'][0] # The first one is the inh spike train w/o synchronous input
    ...
    def process_section(i):
        ...
        if spat_condition == 'clus':
            ...
            spike_train_inh = spike_trains_inh[i]

        elif spat_condition == 'distr':
            spike_train_inh = ast.literal_eval(section_clus['spike_train_bg'])[num_activated_preunit] # the corresponding spike train with the identical number of synchronous inputs

        netstim = h.VecStim()
        netstim.play(h.Vector(spike_train_inh))
        ...
        section_synapse_df.at[section.name, 'netstim'] = netstim
        section_synapse_df.at[section.name, 'spike_train_bg'].append(list(spike_train_inh))
        section_synapse_df.at[section.name, 'netcon'] = netcon
