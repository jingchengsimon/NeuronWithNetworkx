...
def add_background_exc_inputs(section_synapse_df, syn_param_exc, DURATION, FREQ_EXC, 
    ...
    def process_section(i):
        ...
        if section['type'] == 'C':
            spike_train_sync = section['spike_train'][0] # it's the same df, so spike train is still list type not string in csv
            spike_train = np.concatenate((spike_train, np.array(spike_train_sync, dtype=np.int64))) # first list -> ndarray
