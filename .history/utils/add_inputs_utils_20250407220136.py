...
def add_background_exc_inputs(section_synapse_df, syn_param_exc, DURATION, FREQ_EXC, 
    ...
    sec_syn_clus_exc_df = section_synapse_df[section_synapse_df['type'] == 'C']
    num_syn_clus_exc = len(sec_syn_clus_exc_df)
    ...
    def process_section(i):
        ...
        if section['type'] == 'C':
            spike_train_sync = section['spike_train'][0]
            spike_train = np.sort(np.concatenate((spike_train, spike_train_sync)))
            print(f'spike_train for synchronous synapses {i}: {spike_train}')
            section_synapse_df.at[section.name, 'spike_train'] = [] # clear the spike train for synapses with synchronous inputs
    ...
    with ThreadPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
        list(tqdm(executor.map(process_section, range(num_syn_clus_exc)), total=num_syn_clus_exc))
        # executor.map(process_section, range(num_syn_clus_exc))
