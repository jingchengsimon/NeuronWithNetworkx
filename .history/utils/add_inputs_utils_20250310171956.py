...
def add_background_exc_inputs(section_synapse_df, syn_param_exc, DURATION, FREQ_EXC, 
                              input_ratio_basal_apic, bg_exc_channel_type, initW, num_func_group, lock):
    ...
    syn_w_distr = np.random.lognormal(mean=mu, sigma=sigma, size=50000)
    ...
    def process_section(i):
        ...
        if section['synapse'] is None:
            ...
            if bg_exc_channel_type == 'Exp2Syn':
                ...
                initW_distr = np.random.choice(syn_w_distr, 1)[0]
        ...
        section_synapse_df.at[section.name, 'spike_train'] = list(spike_train) 
...
def add_background_inh_inputs(section_synapse_df, syn_param_inh, DURATION, FREQ_INH, 
    ...
    spike_counts_basal_inh = np.random.poisson(lambda_array/1000, size=(int(num_syn_basal_inh/1), DURATION))
    spike_counts_apic_inh = np.random.poisson(lambda_array/(input_ratio_basal_apic*1000), size=(int(num_syn_apic_inh/1), DURATION))
    spike_counts_soma_inh = np.random.poisson(lambda_array/1000, size=(int(num_syn_soma_inh/1), DURATION))
    ...
    for j in range(num_clusters):
        ...
        for i in range(num_syn_clustered):
            ...
            if section['synapse'] is None:
                ...
                initW_distr = np.random.choice(syn_w_distr, 1)[0]  # variedW
            ...
            section_synapse_df.at[section.name, 'spike_train'] = spt_unitsection_synapse_df.at[section.name, 'netcon'] = netcon
            # time.sleep(0.01)
