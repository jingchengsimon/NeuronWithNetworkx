...
def add_background_inh_inputs(section_synapse_df, syn_param_inh, DURATION, FREQ_INH, 
    ...
    spike_time_sync = section_synapse_df[section_synapse_df['type'].isin(exc_types)]['spike_train']
    total_spikes = np.zeros(DURATION)  # 初始化总spike数数组

    for spike_times_bg_list, spike_times_sync_list in zip(spike_times_bg, spike_time_sync):
