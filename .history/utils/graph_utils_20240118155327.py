...
def create_graph(all_sections, section_df):
    ...
    section_df.to_csv("cell1.csv", encoding='utf-8', index=False)
    ...
    G = nx.from_pandas_dataframe(section_df, source='parent_id', target='section_id'create_using=Graphtype, , edge_attr=True)
    
    Data = open('cell1.csv', "r")
    next(Data, None)  # skip the first line in the input file
    DiG = nx.parse_edgelist(Data, delimiter=',', create_using=DiGraphtype,
                                    nodetype=int, data=(('parent_name', str), ('section_name', str),
                                                    ('length', float), ('section_type', str)))
