    ...
    def process_section(i):
        ...
        if section['synapse'] is None:
            ...
            if bg_exc_channel_type == 'Exp2Syn':
                ...
                synapse = AMPANMDA(initW_distr, section['loc'], section['section_synapse'], bg_exc_channel_type)
        ...
        if spat_condition == 'clus':
            ...
            bg_spike_train = np.where(counts >= 1)[0] # ndarray
            ...
            mask = rnd.choice([True, False], size=bg_spike_train.shape, p=[0.5, 0.5])
            bg_spike_train = bg_spike_train[mask]

        elif spat_condition == 'distr':
            bg_spike_train = ast.literal_eval(section_clus['bg_spike_train'])[0]

        if i < 10:
            # test rnd
            print(f'syn weight {i}: {initW_distr}')
            print(f'background spike train {i}: {bg_spike_train}')
            
        netstim = h.VecStim()
        netstim.play(h.Vector(bg_spike_train))
        ...
        section_synapse_df.at[section.name, 'spike_train'].append(list(bg_spike_train)) 
...
def add_background_inh_inputs(section_synapse_df, syn_param_inh, DURATION, FREQ_INH, 
    ...
    def process_section(i):
        ...
        if spat_condition == 'clus':
            ...
            bg_spike_train = np.where(counts >= 1)[0] 
        ...
        section_synapse_df.at[section.name, 'netstim'] = netstim
        section_synapse_df.at[section.name, 'bg_spike_train'].append(list(bg_spike_train))
        section_synapse_df.at[section.name, 'netcon'] = netcon
...
def add_clustered_inputs(section_synapse_df, num_clusters, basal_channel_type, initW, spt_unit_list, rnd):  
    ...
    for j in range(num_clusters):
        ...
        for i in range(num_syn_clustered):
            ...
            if section['synapse'] is None:
                ...
                synapse = AMPANMDA(initW_distr, section['loc'], section['section_synapse'], basal_channel_type)
            else:
                synapse = section['synapse']
