...
class CellWithNetworkx:
        ...
        for i in range(self.num_clusters):
            ...
            while exceed_flag == False:
                ...
                if num_syn_per_clus > 1:
                    ...
                    try:
                        dis_mark_from_ctr = np.sort(self.rnd.exponential(cluster_radius, num_syn_per_clus - 1))
                        max_dis_mark_from_ctr = np.sort(self.rnd.exponential(cluster_radius, max_num_syn_per_clus - 1))
                    except ValueError:
                        dis_mark_from_ctr = np.sort(self.rnd.exponential(cluster_radius, 0))
                        max_dis_mark_from_ctr = np.sort(self.rnd.exponential(cluster_radius, 0))
                    ...
                    # while len(dis_syn_from_ctr) < num_syn_per_clus - 1:
                        ...
                        if list(self.DiG.successors(syn_suc_sec_id)):
                            ...
                            try:
                                syn_suc_sec = sec_syn_bg_exc_ordered_df[sec_syn_bg_exc_ordered_df['section_id_synapse'] == syn_suc_sec_id]['section_synapse'].values[0]
                                syn_suc_surround_ctr = sec_syn_bg_exc_ordered_df[sec_syn_bg_exc_ordered_df['section_id_synapse'] == syn_suc_sec_id]
                        ...
                        if list(self.DiG.predecessors(syn_pre_sec_id)) not in ([], [0], [121]):
                            ...
                            try:
                                syn_pre_sec = sec_syn_bg_exc_ordered_df[sec_syn_bg_exc_ordered_df['section_id_synapse'] == syn_pre_sec_id]['section_synapse'].values[0]
                                syn_pre_surround_ctr = sec_syn_bg_exc_ordered_df[sec_syn_bg_exc_ordered_df['section_id_synapse'] == syn_pre_sec_id]
                        ...
                        suc_exceed_flag = all(sec_id in np.unique(sec_syn_bg_exc_ordered_df['section_id_synapse']) for sec_id in list(self.DiG.successors(syn_suc_sec_id)))
                        pre_exceed_flag = all(sec_id in np.unique(sec_syn_bg_exc_ordered_df['section_id_synapse']) for sec_id in list(self.DiG.predecessors(syn_pre_sec_id)))
                        exceed_flag = (list(self.DiG.successors(syn_suc_sec_id)) not in np.unique(sec_syn_bg_exc_ordered_df['section_id_synapse'])) and
                                      (list(self.DiG.predecessors(syn_pre_sec_id)) not in np.unique(sec_syn_bg_exc_ordered_df['section_id_synapse'])) and
                                      (len(dis_syn_from_ctr) < num_syn_per_clus - 1)
                        
                max_clus_mem_idx = distance_synapse_mark_compare(dis_syn_from_ctr, max_dis_mark_from_ctr)
                clus_mem_idx = self.rnd.choice(max_clus_mem_idx, num_syn_per_clus - 1, replace=False)

                # assign the surround as clustered synapse
                self.section_synapse_df.loc[syn_surround_ctr.iloc[clus_mem_idx].index, 'type'] = 'C'
    ...
    def add_single_synapse(self, num_syn, region, sim_type):
        ...
        def generate_synapse(_):
            ...
            with self.lock:
                # self.section_synapse_df = pd.concat([self.section_synapse_df, data_to_append], ignore_index=True)
...
def build_cell(**params):
    ...
    folder_tag = str(int(folder_tag) % 49) if int(folder_tag) % 49 != 0 else '49'
...
#                 futures.append(executor.submit(build_cell, **params_with_epoch))
#             # Optionally, wait for all processes to complete
#             for future in futures:
#                 future.result()  # This will re-raise exceptions if any occurred in child processes
...
#     processes = []
#     for params in parameters_list:
#         for epoch in range(1, 2): # run different assignments with same parameters for 5 times

#             params_with_epoch = params.copy()  # Create a copy of the params dictionary
#             params_with_epoch['epoch'] = epoch  # Add the epoch key-value pair
#             process = multiprocessing.Process(target=build_cell, kwargs=params_with_epoch)
