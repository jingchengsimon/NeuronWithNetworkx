...
class CellWithNetworkx:
    ...
    def add_inputs(self, folder_path, input_ratio_basal_apic, bg_exc_channel_type, initW, inh_delay, num_trials):
        ...
        num_exc_syn_on_basal_sec = self.section_synapse_df[(self.section_synapse_df['section_id_synapse'] == 70) &
        ...
        num_exc_syn_on_tuft_sec = self.section_synapse_df[(self.section_synapse_df['section_id_synapse'] == 154) &
                                                        (self.section_synapse_df['type'] == 'A')].shape[0]
        self.tuft_bg_i_nmda_array = np.zeros((num_exc_syn_on_tuft_sec, num_time_points, num_aff_fibers, num_trials))
        self.tuft_bg_i_ampa_array = np.zeros((num_exc_syn_on_tuft_sec, num_time_points, num_aff_fibers, num_trials))
                
        clustered_sec = self.section_synapse_df[self.section_synapse_df['type'] == 'C']['section_synapse'].iloc[:1]
        num_exc_syn_on_clus_sec = self.section_synapse_df[(self.section_synapse_df['section_synapse'].isin(clustered_sec)) & 
                                                           (self.section_synapse_df['type'] == 'A')].shape[0]
        self.clus_bg_i_nmda_array = np.zeros((num_exc_syn_on_clus_sec, num_time_points, num_aff_fibers, num_trials))
        self.clus_bg_i_ampa_array = np.zeros((num_exc_syn_on_clus_sec, num_time_points, num_aff_fibers, num_trials))
    ...
    def run_simulation(self, num_aff_fiber, num_trial):
        ...
        basal_v = h.Vector().record(self.complex_cell.dend[470](0.5)._ref_v) # the 5th dendrite (tip), L: 84.2, order: 3
        ...
        exc_syn_on_basal_sec = self.section_synapse_df[(self.section_synapse_df['section_id_synapse'] == 70) &
        ...
        exc_syn_on_tuft_sec = self.section_synapse_df[(self.section_synapse_df['section_id_synapse'] == 154) &
                                                      (self.section_synapse_df['type'] == 'A')]['synapse']
        tuft_bg_i_nmda_list = []  
        tuft_bg_i_ampa_list = []
        ...
        for exc_syn_id in range(len(exc_syn_on_basal_sec)):
            self.basal_bg_i_nmda_array[exc_syn_id, :, num_aff_fiber, num_trial] = np.array(basal_bg_i_nmda_list[exc_syn_id])
            self.basal_bg_i_ampa_array[exc_syn_id, :, num_aff_fiber, num_trial] = np.array(basal_bg_i_ampa_list[exc_syn_id])

        for exc_syn_id in range(len(exc_syn_on_tuft_sec)):
            self.tuft_bg_i_nmda_array[exc_syn_id, :, num_aff_fiber, num_trial] = np.array(tuft_bg_i_nmda_list[exc_syn_id])
            self.tuft_bg_i_ampa_array[exc_syn_id, :, num_aff_fiber, num_trial] = np.array(tuft_bg_i_ampa_list[exc_syn_id])

        for exc_syn_id in range(len(exc_syn_on_clus_sec)):
