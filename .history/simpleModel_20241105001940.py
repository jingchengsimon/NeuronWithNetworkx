...
from utils.dist_to_soma import recur_dist_to_soma, recur_dist_to_root
...
class CellWithNetworkx:
    ...
    def assign_clustered_synapses(self, basal_channel_type, sec_type, dis_to_root, 
        ...
        # Extract distances
        basal_distance = self.section_synapse_df[(self.section_synapse_df['region'] == 'basal') & (section_synapse_df['type'] == 'A')]['distance_to_soma'].values
        tuft_distance = self.ection_synapse_df[(self.section_synapse_df['distance_to_tuft'] != -1) & (section_synapse_df['type'] == 'A')]['distance_to_tuft'].values
        ...
        indices_basal = [sorted_basal_distances[threshold - 1] for threshold in thresholds if threshold <= len(sorted_basal_distances)]
        indices_tuft = [sorted_tuft_distances[threshold - 1] for threshold in thresholds if threshold <= len(sorted_tuft_distances)]
        ...
        for i in range(self.num_clusters):
            ...
            if sec_type == 'basal':
                sec_syn_bg_exc_ordered_df = self.section_synapse_df[
                    ...
                    (self.section_synapse_df['type'] == 'A') &
                    (self.section_synapse_df['distance_to_soma'].between(137, 176))] 
                    # (self.section_synapse_df['section_synapse'].isin(sections_k_distance)) & ]
            elif sec_type == 'apical':
                sec_syn_bg_exc_ordered_df = self.section_synapse_df[
                    (self.section_synapse_df['section_id_synapse'] in self.sec_tuft_idx) &
                    (self.section_synapse_df['type'] == 'A') &
                    (self.section_synapse_df['distance_to_tuft'].between(dis_to_root*100, (dis_to_root+1)*100))]
    ...
    def add_inputs(self, folder_path, simu_condition, input_ratio_basal_apic, bg_exc_channel_type, initW, inh_delay, num_trials):
        ...
        for num_activated_preunit in self.num_activated_preunit_list[15:]: # 40
