...
class CellWithNetworkx:
    ...
    def assign_clustered_synapses(self, num_clusters, cluster_radius, distance_to_soma, num_conn_per_preunit, num_syn_per_cluster, b):
    ...
    def add_inputs(self, folder_path):
        ...
        for num_syn_to_get_input in range(self.num_syn_per_cluster):
            self._add_clustered_inputs(spt_unit_list, num_syn_to_get_input, basal_channel_type)
            ...
            time_v, dend_v = self._run_simulation(ori_dg, stim_id, stim_index)
            dend_v_peak = np.max(np.array(dend_v)[20000:24000])
            print('peak of dend_v:', dend_v_peak)
        ...
        plt.figure()
        for i in range(0, len(self.dend_v_list), 4):
            plt.plot(time_v, self.dend_v_list[-(i+1)], label=len(self.dend_v_list)-i)
        plt.legend()
        plt.xlabel('Time (ms)')
    ...
    def _run_simulation(self, ori_dg, stim_id, stim_index):
        ...
        self.dend_v_list.append(np.array(dend_v))
        # print(np.array(dend_v)[20000:20100])
    ...
    def _add_clustered_inputs(self, spt_unit_list, num_syn_to_get_input, basal_channel_type):  
        ...
        for j in range(self.num_clusters):
            ...
            sec_syn_clustered_df = self.section_synapse_df[(self.section_synapse_df['type'] == 'C') & 
                                                           (self.section_synapse_df['cluster_id'] == j)]
            num_syn_clustered = len(sec_syn_clustered_df)
            ...
            for i in tqdm(range(num_syn_clustered)):
                ...
                if section['synapse'] is None:
                    ...
                    synapse = self.AMPANMDA(syn_params, section['loc'], section['section_synapse'], basal_channel_type)
    ...
    def AMPANMDA(self, syn_params, sec_x, sec_id, basal_channel_type):
