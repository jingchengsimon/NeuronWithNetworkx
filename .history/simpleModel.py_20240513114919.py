...
for i in range(1):
    ...
    seg_for_synapse = seg_for_synapse[int(seg_for_synapse.sec.nseg/2)]
    loc = seg_for_synapse.x
    sec = seg_for_synapse.sec
    synapses_list.append(AMPANMDA(syn_params, loc, sec, 'AMPANMDA'))
...
basal_ctr = seg_for_synapse
syn_ctr = synapses_list[0]

dend_v = h.Vector().record(basal_ctr._ref_v)
...
plt.plot(time_v, dend_v, label='Dendrite')
plt.legend()
plt.xlabel('Time (ms)')
plt.ylabel('Voltage (mV)')
plt.title('Soma and Dendrite Voltage')

plt.subplot(2,1,2)
plt.plot(time_v, 1000*dend_i, label='Total')
plt.plot(time_v, 1000*dend_i_nmda, label='NMDA')
plt.plot(time_v, 1000*dend_i_nmda, label='AMPA')
plt.legend()
plt.xlabel('Time (ms)')
