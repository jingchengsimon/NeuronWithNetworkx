...
def add_single_synapse(section_df, num_syn, region, sim_type):
        sections = sections_basal if region == 'basal' else self.sections_apical
        # e_syn, tau1, tau2, syn_weight = self.syn_param_exc if sim_type == 'exc' else self.syn_param_inh
        type = 'A' if sim_type == 'exc' else 'B'
        
        if region == 'basal':
            section_length = np.array(section_df.loc[self.section_df['section_type'] == 'dend', 'length'])  
        else:
            section_length = np.array(section_df.loc[self.section_df['section_type'] == 'apic', 'length'])
