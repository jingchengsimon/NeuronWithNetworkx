...
class CellWithNetworkx:
    ...
    def add_inputs(self, folder_path):
        ...
        for num_syn_to_get_input in range(self.num_syn_per_cluster):
            ...
            self.nmda_i_array = np.zeros((self.num_clusters, num_time_points, self.num_syn_per_cluster, num_trials))
            self.ampa_i_array = np.zeros((self.num_clusters, num_time_points, self.num_syn_per_cluster, num_trials))
            self.soma_v_array = np.zeros((num_time_points, num_trials))

            # For analysing the supra-linearity by NMDA
            # time_v, dend_v, soma_v = 
            self.run_simulation(ori_dg, stim_id, stim_index, num_syn_to_get_input, folder_path)
    ...
    def run_simulation(self, ori_dg, stim_id, stim_index, num_syn_to_get_input, folder_path):
