...
class CellWithNetworkx:
    def __init__(self, swc_file, bg_exc_freq, bg_inh_freq, SIMU_DURATION, STIM_DURATION):
        ...
        h.v_init = -70 #self.complex_cell.soma[0].e_pas # -90 mV
        ...
        self.syn_param_inh = [-86, 1, 8, 0.00008] #->0.00008 uS = 0.08 nS
        ...
        self.section_synapse_df = pd.DataFrame(columns=['section_id_synapse',
                                                ...
                                                'region',
                                                'spike_train',
                                                'branch_idx',], dtype=object) # for adding vecstim of different orientation
    ...
    def assign_clustered_synapses(self, basal_channel_type, sec_type, dis_to_root, 
        ...
        if sec_type == 'basal':
            num_syn_thres = [1000 + i * 1000 for i in range(2)]
    ...
    def run_simulation(self, num_stim, num_aff_fiber, num_trial, folder_path):
        ...
        vc.dur1 = 1e9  # Long duration to hold the voltage
        vc.amp1 = -70   # Holding voltage at 60 mV
        soma_i = h.Vector().record(vc._ref_i)
        soma_i_pas = h.Vector().record(self.complex_cell.soma[0](0.5)._ref_i_pas)  # Recording current
        soma_i_cap = h.Vector().record(self.complex_cell.soma[0](0.5)._ref_i_cap)  # Recording current
        ...
        h.run()
        print(f"Simulation time: {time.time() - time_start}")
        ...
        with self.lock:
            ...
            self.soma_i_array[:, num_stim, num_aff_fiber, num_trial] = np.array(soma_i_pas) + np.array(soma_i_cap)
...
def build_cell(**params):
    ...
    simu_folder = sec_type + '_range' + str(distance_to_root) + '_' + spat_condtion + '_' + simu_condition + '_variedW_EPSCtest' # + '_ratio1' + '_exc1.1-1.3' + '_inh4' + '_failprob0.5' + '_funcgroup10'
