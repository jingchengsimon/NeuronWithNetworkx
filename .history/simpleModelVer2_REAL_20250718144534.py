...
from utils.add_inputs_utils import add_background_exc_inputs_2, add_background_inh_inputs, add_clustered_inputs
...
class CellWithNetworkx:
    def __init__(self, swc_file, bg_exc_freq, bg_inh_freq, SIMU_DURATION, STIM_DURATION, epoch_idx):
        ...
        self.all_segments_dend = np.array([seg for sec in self.sections_basal + self.sections_apical for seg in sec])
        self.all_segments_dend_dict = {i: seg for i, seg in enumerate(self.all_segments_dend)}
        ...
        data_list = []
        for _, seg in all_segments_noaxon_dict.items():
            data_list.append({
                'section_name': seg.sec.name(),
                'x_position': seg.x
                # 可以添加更多你需要的属性
            })
        self.all_segments_dend_df = pd.DataFrame(data_list)

        # # 保存为 CSV
        # df.to_csv('all_segments_noaxon.csv', index=False)
    ...
    def add_inputs(self, folder_path, simu_condition, input_ratio_basal_apic, bg_exc_channel_type, initW, num_func_group, inh_delay, num_trials):
        ...
        if simu_condition == 'invivo':
            ...
            add_background_exc_inputs_2(self.section_synapse_df, self.all_segments_dend, self.syn_param_exc, self.SIMU_DURATION, self.FREQ_EXC, 
...
def build_cell(**params):
    ...
    if basal_channel_type == 'AMPANMDA':                                                                                
        simu_folder = sec_type + '_range' + str(distance_to_root) + '_' + spat_condtion + '_' + simu_condition + '_NATURAL_exc1.3_funcgroup639' #_variedW_tau43_addNaK_woAP+Ca_aligned_varyinh' # + '_ratio1' + '_exc1.1-1.3' + '_inh4' + '_failprob0.5' + '_funcgroup10'
