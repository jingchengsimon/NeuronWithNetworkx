...
class CellWithNetworkx:
    ...
    def add_inputs(self, folder_path, input_ratio_basal_apic, bg_exc_channel_type, initW, inh_delay, num_trials):
        ...
        num_exc_syn_on_basal_sec = self.section_synapse_df[(self.section_synapse_df['section_id_synapse'] == 71) &
        ...
        num_exc_syn_on_tuft_sec = self.section_synapse_df[(self.section_synapse_df['section_id_synapse'] == 152) &
                                                        (self.section_synapse_df['type'] == 'A')].shape[0]
    ...
    def run_simulation(self, num_aff_fiber, num_trial):
        ...
        self.basal_bg_i_nmda_array[:, num_aff_fiber, num_trial] = np.sum(np.array(basal_bg_i_nmda_list), axis=0)
        self.basal_bg_i_ampa_array[:, num_aff_fiber, num_trial] = np.sum(np.array(basal_bg_i_ampa_list), axis=0)

        self.tuft_bg_i_ampa_array[:, num_aff_fiber, num_trial] = np.sum(np.array(tuft_bg_i_ampa_list), axis=0)
        self.tuft_bg_i_nmda_array[:, num_aff_fiber, num_trial] = np.sum(np.array(tuft_bg_i_nmda_list), axis=0)
