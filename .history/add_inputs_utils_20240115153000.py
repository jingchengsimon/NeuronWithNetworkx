...
def add_background_inh_inputs(section_synapse_df, syn_param_inh, time_interval, DURATION, FREQ_INH, lock):  
    exc_types = ['A','C']
    ...
    spike_counts_basal_inh = np.random.poisson(lambda_array, size=(self.num_syn_basal_inh, self.DURATION))
    spike_counts_apic_inh = np.random.poisson(lambda_array, size=(self.num_syn_apic_inh, self.DURATION))
...
def add_clustered_inputs(section_synapse_df, syn_param_exc, spike_interval, lock, spt_unit_list, num_syn_to_get_input, basal_channel_type):  
