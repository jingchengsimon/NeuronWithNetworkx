...
class CellWithNetworkx:
    ...
    def assign_clustered_synapses(self, basal_channel_type, sec_type,
        ...
        for i in range(num_clusters):
            ...
            def recursive_distance_to_soma(sec, initial=True):
                # If this is the first (initial) section, use syn_ctr['loc'] to calculate partial length
                if initial:
                ...
                if h.SectionRef(sec).has_parent():
                    ...
                    # Add the distance of the current section to the recursive call for the parent
                    return distance + recursive_distance_to_soma(parent_sec, initial=False)
                else:
                    # If there is no parent, we have reached the soma
                    return distance + sec.L * 0.5
