...
class CellWithNetworkx:
    ...
    def assign_clustered_synapses(self, basal_channel_type, sec_type, dis_to_root, 
        ...
        for i in range(self.num_clusters):
            # Define the concentration level for clus (8 clus on 8 branches / 1 branch)
            sec_syn_bg_exc_df = self.section_synapse_df[(self.section_synapse_df['type'] == 'A') &
                                                        (self.section_synapse_df['branch_idx'] == i)]
    ...
    def add_single_synapse(self, num_syn, region, sim_type):
        ...
        def generate_synapse(_):
            ...
            section_id_synapse = self.section_df[self.section_df['section_name'] == section_name]['section_id'].values[0]
            # self.section_df.loc[self.section_df['section_name'] == section_name, 'section_id'].values[0]
            branch_idx = self.section_df.loc[self.section_df['section_name'] == section_name, 'branch_idx'].values[0]self.section_df[self.section_df['section_name'] == section_name]['branch_idx'].values[0]   
            # 
...
def build_cell(**params):
    ...
    simu_folder = sec_type + '_range' + str(distance_to_root) + '_' + spat_condtion + '_' + simu_condition + '_variedW_concentest' # + '_ratio1' + '_exc1.1-1.3' + '_inh4' + '_failprob0.5' + '_funcgroup10'
