...
def build_cell(**params):
    ...
    simu_folder = sec_type + '_range' + str(distance_to_root) + '_' + spat_condtion + '_' + simu_condition + '_variedW_tau90_addNaK_overallbranchtest_varynumsyn_step2' # + '_ratio1' + '_exc1.1-1.3' + '_inh4' + '_failprob0.5' + '_funcgroup10'
...
if __name__ == "__main__":
    ...
    with ProcessPoolExecutor(max_workers=1) as executor:  # 根据CPU核心数调整
        executor.map(run_combination, combinations)

    processes = []
    for sec_type, spat_cond, dis_to_root, epoch in itertools.product(
        ['basal', 'apical'],
        ['clus', 'distr'],
        [0, 1, 2],
        range(1, 11)
    ):
        p = multiprocessing.Process(target=run_combination, args=(sec_type, spat_cond, dis_to_root, epoch))
        p.start()
        processes.append(p)
