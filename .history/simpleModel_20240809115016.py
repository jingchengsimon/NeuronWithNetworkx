...
class CellWithNetworkx:
    def __init__(self, swc_file, bg_exc_freq, bg_inh_freq, DURATION):
        ...
        self.bg_exc_i_nmda_array = None
        self.bg_exc_i_ampa_array = None
    ...
    def add_inputs(self, folder_path, input_ratio_basal_apic, bg_exc_channel_type, initW, inh_delay, num_trials):
        ...
        # num_activated_preunit_list = range(0, 100+5, 5)
        ...
        self.trunk_v_array = np.zeros((num_time_points, num_aff_fibers, num_trials))

        self.basal_v_array = np.zeros((num_time_points, num_aff_fibers, num_trials))
        self.tuft_v_array = np.zeros((num_time_points, num_aff_fibers, num_trials))
        ...
        num_exc_syn_on_basal_sec = self.section_synapse_df[(self.section_synapse_df.index == 5) &
                                                       (self.section_synapse_df['type'] == 'A')].shape[0]
        self.basal_exc_i_nmda_array = np.zeros((num_exc_syn_on_basal_sec, num_time_points, num_aff_fibers, num_trials))
        self.basal_exc_i_ampa_array = np.zeros((num_exc_syn_on_basal_sec, num_time_points, num_aff_fibers, num_trials))
        num_exc_syn_on_tuft_sec = self.section_synapse_df[(self.section_synapse_df.index == 129) &
                                                        (self.section_synapse_df['type'] == 'A')].shape[0]
        self.tuft_exc_i_nmda_array = np.zeros((num_exc_syn_on_tuft_sec, num_time_points, num_aff_fibers, num_trials))
        self.tuft_exc_i_ampa_array = np.zeros((num_exc_syn_on_tuft_sec, num_time_points, num_aff_fibers, num_trials))
                
        clustered_sec = self.section_synapse_df[self.section_synapse_df['type'] == 'C']['section_synapse'].iloc[:1]
        num_exc_syn_on_clustered_sec = self.section_synapse_df[(self.section_synapse_df['section_synapse'].isin(clustered_sec)) & 
                                                           (self.section_synapse_df['type'] == 'A')].shape[0]
        self.bg_exc_i_nmda_array = np.zeros((num_exc_syn_on_clustered_sec, num_time_points, num_aff_fibers, num_trials))
        self.bg_exc_i_ampa_array = np.zeros((num_exc_syn_on_clustered_sec, num_time_points, num_aff_fibers, num_trials))
        ...
        np.save(os.path.join(folder_path,'basal_exc_i_nmda_array.npy'), self.basal_exc_i_nmda_array)
        np.save(os.path.join(folder_path,'basal_exc_i_ampa_array.npy'), self.basal_exc_i_ampa_array)
        np.save(os.path.join(folder_path,'tuft_exc_i_nmda_array.npy'), self.tuft_exc_i_nmda_array)
        np.save(os.path.join(folder_path,'tuft_exc_i_ampa_array.npy'), self.tuft_exc_i_ampa_array)

        np.save(os.path.join(folder_path,'bg_exc_i_nmda_array.npy'), self.bg_exc_i_nmda_array)
        np.save(os.path.join(folder_path,'bg_exc_i_ampa_array.npy'), self.bg_exc_i_ampa_array)
    ...
    def run_simulation(self, num_aff_fiber, num_trial):
        ...
        basal_v = h.Vector().record(self.complex_cell.dend[4](0.5)._ref_v) # the 5th dendrite (tip), L: 84.2, order: 3
        tuft_v = h.Vector().record(self.complex_cell.apic[44](0.5)._ref_v) # the 129th dendrite (tip), L: 92.6, order: 3

        # Record summed local background NMDA current at the basal tip branch
        exc_syn_on_basal_sec = self.section_synapse_df[(self.section_synapse_df.index == 5) &
                                                       (self.section_synapse_df['type'] == 'A')]['synapse']
        basal_exc_i_nmda_list = []
        basal_exc_i_ampa_list = []

        for exc_syn in exc_syn_on_basal_sec:
            basal_exc_i_nmda = h.Vector().record(exc_syn._ref_i_NMDA)
            basal_exc_i_ampa = h.Vector().record(exc_syn._ref_i_AMPA)

            basal_exc_i_nmda_list.append(basal_exc_i_nmda)
            basal_exc_i_ampa_list.append(basal_exc_i_ampa)

        # Record summed local background NMDA current at the tuft tip branch
        exc_syn_on_tuft_sec = self.section_synapse_df[(self.section_synapse_df.index == 129) &
                                                      (self.section_synapse_df['type'] == 'A')]['synapse']
        tuft_exc_i_nmda_list = []  
        tuft_exc_i_ampa_list = []

        for exc_syn in exc_syn_on_tuft_sec:
            tuft_exc_i_nmda = h.Vector().record(exc_syn._ref_i_NMDA)
            tuft_exc_i_ampa = h.Vector().record(exc_syn._ref_i_AMPA)

            tuft_exc_i_nmda_list.append(tuft_exc_i_nmda)
            tuft_exc_i_ampa_list.append(tuft_exc_i_ampa)
        
        # Record summed local background NMDA current at the clustered branch
        bg_exc_i_nmda_list = []
        bg_exc_i_ampa_list = []

        clustered_sec = self.section_synapse_df[self.section_synapse_df['type'] == 'C']['section_synapse'].iloc[:1]
        exc_syn_on_clustered_sec = self.section_synapse_df[(self.section_synapse_df['section_synapse'].isin(clustered_sec)) & 
                                                           (self.section_synapse_df['type'] == 'A')]['synapse']
        for exc_syn in exc_syn_on_clustered_sec:
            bg_exc_i_nmda = h.Vector().record(exc_syn._ref_i_NMDA)
            bg_exc_i_ampa = h.Vector().record(exc_syn._ref_i_AMPA)
        ...
        for exc_syn_id in range(len(exc_syn_on_clustered_sec)):
            self.bg_exc_i_nmda_array[exc_syn_id, :, num_aff_fiber, num_trial] = np.array(bg_exc_i_nmda_list[exc_syn_id])
            self.bg_exc_i_ampa_array[exc_syn_id, :, num_aff_fiber, num_trial] = np.array(bg_exc_i_ampa_list[exc_syn_id])
