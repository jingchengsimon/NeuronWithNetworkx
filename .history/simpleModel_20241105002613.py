...
class CellWithNetworkx:
    ...
    def assign_clustered_synapses(self, basal_channel_type, sec_type, dis_to_root, 
        ...
        num_syn_thres = [2000, 4000, 6000, 8000]

        # Get the indices for the thresholds
        dist_thres_basal = [0] + [sorted_basal_distances[threshold - 1] for threshold in num_syn_thres if threshold <= len(sorted_basal_distances)] + []
        dist_thres_tuft = [0] + [sorted_tuft_distances[threshold - 1] for threshold in num_syn_thres if threshold <= len(sorted_tuft_distances)]
        ...
        for i in range(self.num_clusters):
            ...
            if sec_type == 'basal':
                sec_syn_bg_exc_ordered_df = self.section_synapse_df[
                    ...
                    (self.section_synapse_df['type'] == 'A') &
                    (self.section_synapse_df['distance_to_soma'].between(dist_thres_basal[i], dist_thres_basal[i+1]))] 
                    # (self.section_synapse_df['section_synapse'].isin(sections_k_distance)) & ]
            elif sec_type == 'apical':
                sec_syn_bg_exc_ordered_df = self.section_synapse_df[
                    (self.section_synapse_df['section_id_synapse'] in self.sec_tuft_idx) &
                    (self.section_synapse_df['type'] == 'A') &
                    (self.section_synapse_df['distance_to_tuft'].between(dist_thres_tuft[i], dist_thres_tuft[i+1]))]
