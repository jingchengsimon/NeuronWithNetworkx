...
class CellWithNetworkx:
    ...
    def assign_clustered_synapses(self, basal_channel_type, sec_type,
                                  ...
                                  num_syn_per_cluster, num_conn_per_preunit,
                                  folder_path):
    ...
    def add_inputs(self, folder_path, num_trials):
        ...
        num_activated_preunit_list = [1,5,10,25,50,100,150,200]
        num_epochs = len(num_activated_preunit_list)
    ...
    def run_simulation(self, num_per_epoch, trial_idx):
        ...
        self.soma_v_array[:, num_per_epoch, trial_idx] = np.array(soma_v)
        for cluster_id in range(self.num_clusters):
            self.dend_v_array[cluster_id, :, num_per_epoch, trial_idx] = np.array(dend_v_list[cluster_id])
            self.dend_i_array[cluster_id, :, num_per_epoch, trial_idx] = np.array(dend_i_list[cluster_id])
            self.dend_nmda_i_array[cluster_id, :, num_per_epoch, trial_idx] = np.array(dend_i_nmda_list[cluster_id])
            self.dend_ampa_i_array[cluster_id, :, num_per_epoch, trial_idx] = np.array(dend_i_ampa_list[cluster_id])
