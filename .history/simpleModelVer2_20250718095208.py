...
class CellWithNetworkx:
    ...
    def add_inputs(self, folder_path, simu_condition, input_ratio_basal_apic, bg_exc_channel_type, initW, num_func_group, inh_delay, num_trials):
        ...
        for num_stim in range(1, self.num_stim + 1):
            ...
            spt_unit_list = spt_unit_array.tolist()
            if pre_unit_id_first_syn in spt_unit_list:
                spt_unit_list.remove(pre_unit_id_first_syn)
                spt_unit_list = [pre_unit_id_first_syn] + spt_unit_list
            spt_unit_array = np.array(spt_unit_list)
        ...
        self.num_activated_preunit_list = [0, 1, 2, 3, 6, 12, 24, 48, 72] #[self.num_preunit] # for multi-clus
...
def build_cell(**params):
    ...
    if basal_channel_type == 'AMPANMDA':                                                                                
        simu_folder = sec_type + '_range' + str(distance_to_root) + '_' + spat_condtion + '_' + simu_condition + '_multiclus_2' #_variedW_tau43_addNaK_woAP+Ca_aligned_varyinh' # + '_ratio1' + '_exc1.1-1.3' + '_inh4' + '_failprob0.5' + '_funcgroup10'
    elif basal_channel_type == 'AMPA':
            simu_folder = sec_type + '_range' + str(distance_to_root) + '_' + spat_condtion + '_' + simu_condition + '_multiclus_AMPA_2' #variedW_tau43_addNaK_woAP+Ca_aligned_varyinh_AMPA' 
