...
class CellWithNetworkx:
    ...
    def _run_simulation(self, ori_dg, stim_id, stim_index):
        ...
        visualization_params = [ori_dg, stim_id, soma_v, dend_v, apic_v, time_v, spike_times_clustered, dend_i, dend_i_nmda, spike_times]
        self._visualize_simulation(visualization_params)

    def _visualize_simulation(self, visualization_params):
        ori_dg, stim_id, soma_v, dend_v, apic_v, time_v, spike_times_clustered, dend_i, dend_i_nmda, spike_times = visualization_params
        
        # plotting the results
        plt.figure(figsize=(5, 5))
        plt.title(str(ori_dg) + '-' + str(stim_id))
        for i, spike_times_vec in enumerate(spike_times_):
            try:
