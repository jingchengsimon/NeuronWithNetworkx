...
class CellWithNetworkx:
    def __init__(self, swc_file, bg_exc_freq, bg_inh_freq, DURATION):
        ...
        self.basal_exc_i_nmda_array = None
        self.basal_exc_i_ampa_array = None
        self.tuft_exc_i_nmda_array = None
        self.tuft_exc_i_ampa_array = None
    ...
    def add_inputs(self, folder_path, input_ratio_basal_apic, bg_exc_channel_type, initW, inh_delay, num_trials):
        ...
        num_exc_syn_on_clustered_sec = self.section_synapse_df[(self.section_synapse_df['section_synapse'].isin(clustered_sec)) & 
                                                           (self.section_synapse_df['type'] == 'A')].shape[0]
        self.clus_bg_i_nmda_array = np.zeros((num_exc_syn_on_clustered_sec, num_time_points, num_aff_fibers, num_trials))
        self.clus_bg_i_ampa_array = np.zeros((num_exc_syn_on_clustered_sec, num_time_points, num_aff_fibers, num_trials))
    ...
    def run_simulation(self, num_aff_fiber, num_trial):
        ...
        for exc_syn_id in range(len(exc_syn_on_clustered_sec)):
            self.clus_bg_i_nmda_array[exc_syn_id, :, num_aff_fiber, num_trial] = np.array(clus_bg_i_nmda_list[exc_syn_id])
            self.clus_bg_i_ampa_array[exc_syn_id, :, num_aff_fiber, num_trial] = np.array(clus_bg_i_ampa_list[exc_syn_id])
