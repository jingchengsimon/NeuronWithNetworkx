...
class CellWithNetworkx:
    def __init__(self, swc_file, bg_exc_freq, bg_inh_freq, SIMU_DURATION, STIM_DURATION):
        ...
        self.syn_param_exc = [0, 0.3, 1.8] # reversal_potential, tau1, tau2, syn_weight is not here and will be self-defined
    ...
    def assign_clustered_synapses(self, basal_channel_type, sec_type, dis_to_root, 
        ...
        for i in range(self.num_clusters):
            ...
            basal_branch_idx_list = [41,41,41]
    ...
    def add_inputs(self, folder_path, simu_condition, input_ratio_basal_apic, bg_exc_channel_type, initW, num_func_group, inh_delay, num_trials):
        ...
        iter_step = 8
        self.num_activated_preunit_list = range(0, self.num_preunit + 1, 8) # for varynumsyn (add 1 is to allow the last num_preunit to be included)
            ...
            for num_stim in range(self.num_stim):
                for num_trial in range(num_trials): # 20
                    ...
                    for num_stim_idx in range(1, self.num_stim + 1):
                        spt_unit_list = generate_vecstim(self.unit_ids, num_stim_idx, self.stim_time, folder_path)
                        spt_unit_list_list.append(spt_unit_list)
...
def build_cell(**params):
    ...
    simu_folder = sec_type + '_range' + str(distance_to_root) + '_' + spat_condtion + '_' + simu_condition + '_variedW_tau90_addNaK_overallbranchtest_varynumsyn_step8_br41_withAP_multrial' # + '_ratio1' + '_exc1.1-1.3' + '_inh4' + '_failprob0.5' + '_funcgroup10'
...
if __name__ == "__main__":
    ...
    with ProcessPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:  # 根据CPU核心数调整
    ...
    ## Running for heatmap 
    # for sec_type in ['basal', 'apical']: # ['basal', 'apical']
    #     for spat_cond in ['clus', 'distr']: # ['clus', 'distr']
    #         for dis_to_root in [0, 1, 2]: # [0, 1, 2]
    #             params_list = generate_simu_params(sec_type, spat_cond, dis_to_root)
