...
def full_nonlinearity_visualization(exp_list, idx_list, data_list, stack_flag=False, num_epochs=10):
    ...
    plt.savefig(f'/G/results/simulation/202503_Nonlinearity/{anal_loc}_range{range_idx}_{data_list[0]}_gNMDA{g_NMDA}_{spat_cond}_invitro_vairedW_tau90_addNaK.jpg')
        ...
        for spat_cond in ['clus']:
            for range_idx in range(1):
                for attr in ['dend', 'nexus']:
                    full_nonlinearity_visualization([anal_loc+f'_range{range_idx}_{spat_cond}_invivo_vairedW_tau90_addNaK_overallbranchtest_varynumsyn_step2'] * iter_times,
