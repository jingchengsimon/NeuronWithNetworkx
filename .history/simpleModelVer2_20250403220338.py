...
class CellWithNetworkx:
    def __init__(self, swc_file, bg_exc_freq, bg_inh_freq, SIMU_DURATION, STIM_DURATION):
        ...
        self.rnd = np.random.RandomState(42) #int(time.time())) 
        random.seed(42) 
        np.random.seed(42)
    ...
    def assign_clustered_synapses(self, basal_channel_type, sec_type, dis_to_root, 
        ...
        # print(sec_syn_bg_exc_df.iloc[0]['segment_synapse'])
        ...
        for i in range(self.num_clusters):
            ...
            while True:
                loop_count += 1
                rng = np.random.RandomState(42 + loop_count)
    ...
    def add_inputs(self, folder_path, simu_condition, input_ratio_basal_apic, bg_exc_channel_type, initW, num_func_group, inh_delay, num_trials, epoch_idx):
...
def build_cell(**params):
    ...
    simu_folder = sec_type + '_range' + str(distance_to_root) + '_' + spat_condtion + '_' + simu_condition + '_variedW_tau90_addNaK_overallbranchtest_varynumsyn_step8_br41_multrial_withclus_withAP_woCa_withclus' # + '_ratio1' + '_exc1.1-1.3' + '_inh4' + '_failprob0.5' + '_funcgroup10'
