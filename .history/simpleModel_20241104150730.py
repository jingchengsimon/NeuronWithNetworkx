...
class CellWithNetworkx:
    ...
    def add_single_synapse(self, num_syn, region, sim_type):
        ...
        def generate_synapse(_):
            ...
            distance = self.recur_dist_to_soma(segment_synapse, loc)
    ...
    def recur_dist_to_soma(self, sec, loc, initial=True): 
        if h.SectionRef(sec).has_parent():
            sec_len = sec.L * loc if initial else sec.L
            parent_sec = h.SectionRef(sec).parent
            return sec_len + self.recur_dist_to_soma(parent_sec, loc, initial=False)
        else:
