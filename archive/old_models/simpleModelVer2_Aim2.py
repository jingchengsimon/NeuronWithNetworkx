...
from utils.distance_utils import recur_dist_to_soma, recur_dist_to_root
...
class CellWithNetworkx:
    ...
    def add_inputs(self, folder_path, simu_condition, input_ratio_basal_apic, bg_exc_channel_type, initW, num_func_group, inh_delay, num_stim, num_trials):
        ...
        exc_firing_rate_array, inh_firing_rate_array = generate_init_firing(self.section_synapse_df, self.SIMU_DURATION, self.FREQ_EXC, self.FREQ_INH,  
                                                                            self.input_ratio_basal_apic, self.bg_exc_channel_type, self.num_func_group,
                                                                            self.epoch_idx, self.spk_epoch_idx, spat_condition)
        
        # Combine exc and inh firing rate array and save as npy
        firing_rate_array = np.concatenate((exc_firing_rate_array, inh_firing_rate_array), axis=0)
        np.save(os.path.join(folder_path, 'init_firing_rate_array.npy'), firing_rate_array)
...
def create_parser():
    """Create and configure argument parser with REAL default values"""
    parser = argparse.ArgumentParser(description='Neuron simulation parameters (REAL)')
    ...
    # Simulation duration (REAL: 6100ms)
    parser.add_argument('--simu_duration', type=int, default=6100)
    parser.add_argument('--stim_duration', type=int, default=6100)
    ...
    parser.add_argument('--num_syn_per_clus', type=int, default=1)  # REAL: 1
    ...
    # Background input parameters (REAL: bg_exc_freq=1.3)
    parser.add_argument('--bg_exc_freq', type=float, default=1.3)
...
def build_cell(args):
    ...
    if basal_channel_type == 'AMPANMDA':                                                                                
        simu_folder = sec_type + '_range' + str(distance_to_root) + '_' + spat_condtion + '_' + simu_condition + '_Aim2' #_variedW_tau43_addNaK_woAP+Ca_aligned_varyinh' # + '_ratio1' + '_exc1.1-1.3' + '_inh4' + '_failprob0.5' + '_funcgroup10'
    elif basal_channel_type == 'AMPA':
            simu_folder = sec_type + '_range' + str(distance_to_root) + '_' + spat_condtion + '_' + simu_condition + '_Aim2_AMPA' #variedW_tau43_addNaK_woAP+Ca_aligned_varyinh_AMPA' 
    ...
    # Note: This file uses old CellWithNetworkx that takes epoch_idx, not synapse_pos_seed/spike_gen_seed
...
def run_combination(combination_args):
    ...
    sec_type, spat_cond, dis_to_root, epoch = combination_args
