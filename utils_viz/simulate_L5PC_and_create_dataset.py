...
def generate_input_spike_trains_for_simulation(sim_duration_ms, basal_seg_length_um, apical_seg_length_um, min_seg_length_um,
    ...
    ex_inst_spike_prob = np.random.exponential(scale=ex_spike_rate_per_seg_per_1ms)#seg_inst_rate_ex_smoothed)
    ex_spikes_bin      = np.random.rand(ex_inst_spike_prob.shape[0], ex_inst_spike_prob.shape[1]) < ex_inst_spike_prob
    
    plt.figure(figsize=(8, 4))
    plt.plot(np.mean(ex_spike_rate_per_seg_per_1ms[:len(adjusted_basal_length_um)], axis=0), color='b', label='basal')
    plt.plot(np.mean(ex_spike_rate_per_seg_per_1ms[len(adjusted_basal_length_um):], axis=0), color='r', label='apical')
    plt.xlabel('Time (ms)')
    plt.ylabel('Firing rate (Hz)')
    plt.title('Excitatory Spike Rate')
    plt.legend()

    spike_count = ex_spikes_bin.sum()
    num_syn = 8000

    avg_firing_rate = spike_count / (totalSimDurationInSec * num_syn)
    print(f"keep_inst_rate_const_for_ms: {keep_inst_rate_const_for_ms}")
    print(f"spike_count: {spike_count}")
    print(f"平均firing rate: {avg_firing_rate:.4f} Hz")

    spike_rows_ex, spike_cols_ex = np.where(ex_spikes_bin)
    plt.figure(figsize=(8, 4))
    for i in range(len(adjusted_basal_length_um)):
        plt.vlines(spike_cols_ex[spike_rows_ex == i], spike_rows_ex[spike_rows_ex == i], spike_rows_ex[spike_rows_ex == i] + 1, color='b')
    for i in range(len(adjusted_apical_length_um)):
        plt.vlines(spike_cols_ex[spike_rows_ex == i + len(adjusted_basal_length_um)], spike_rows_ex[spike_rows_ex == i + len(adjusted_basal_length_um)], spike_rows_ex[spike_rows_ex == i + len(adjusted_basal_length_um)] + 1, color='r')
    plt.xlabel('Time (ms)')
    plt.ylabel('Segment index')
    plt.title('Exponential Excitatory Spikes Raster')

    from utils.generate_pink_noise import visualize_pink_noise
    visualize_pink_noise(sim_duration_ms)
    # # ex_inst_spike_prob = np.random.exponential(scale=ex_spike_rate_per_seg_per_1ms)
    # # ex_spikes_bin      = np.random.rand(ex_inst_spike_prob.shape[0], ex_inst_spike_prob.shape[1]) < ex_inst_spike_prob
    
    # ex_spikes_bin = np.random.poisson(lam=ex_spike_rate_per_seg_per_1ms)
    # ex_spikes_bin = (ex_spikes_bin > 0).astype(int)  # 只关心有无spike
    ...
    # plt.xlabel('Time (ms)')
    # plt.ylabel('Segment index')
    # plt.title('Poisson Excitatory Spikes Raster')
    ...
    # plt.vlines(spike_cols_inh, spike_rows_inh, spike_rows_inh + 1, color='r')
    # plt.xlabel('Time (ms)')
    # plt.ylabel('Segment index')
