...
try:
    ...
    # If relative import fails, try importing from neuron_reduce package
    from neuron_reduce.reducing_methods import (reduce_subtree,
                                                 reduce_synapse,
                                                 measure_input_impedance_of_subtree,
                                                 CableParams,
                                                 SynapseLocation,
...
def subtree_reductor(original_cell,
    ...
    with push_section(cell.hoc_model.soma[0]):
        h.delete_section()
    if return_seg_to_seg:
        return cell, new_synapses_list, netcons_list, original_seg_to_reduced_seg_text, new_cables_nsegs
    else:
        return cell, new_synapses_list, netcons_list, new_cables_nsegs
