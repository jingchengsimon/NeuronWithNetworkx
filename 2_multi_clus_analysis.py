...
def calculate_mag_values(folder_path, num_epochs=1, data_type='', attr_type='Peak'):
    t = 500
    dt = 1/40000
    t_start, t_end = (t-50)*40, (t+150)*40
    ...
    for epoch in range(1, num_epochs+1):
        ...
        if data_type == 'Soma':
            try:
                ...
                root_v_list.append(np.mean(soma[t_start:t_end, -1, :],axis=1)-soma_baseline)
            ...
            try:
                ...
                root_v_list.append(np.mean(apic_v[t_start:t_end, -1, :],axis=1)-nexus_baseline)
            ...
            try:
                ...
                root_v_list.append(np.mean(dend_v[:, t_start:t_end, -1, :], axis=(0, 2))-dend_baseline)
...
def calculate_ratio_and_draw_heatmap(mag_val_mat1, mag_val_mat2, 
    ...
    res_path = os.path.join(root_folder_path, '202505_Heatmap')
    if not os.path.exists(res_path):
        os.makedirs(res_path)
    ...
    for i in range(num_rows):
        for j in range(num_cols):
            ...
            x_start, x_end = -0.02, 0.10
            ax.set_xlim(x_start, x_end)
            ax.set_ylim(-15, 70)
            # ax.set_yticks([-10, 30, 70])
            ax.hlines([30, 70], x_start, x_end, color='k', linestyle='--', linewidth=1)
    ...
    plt.tight_layout()
    plt.savefig(f'{res_path}/{title}_root_v.png')
...
for rec_loc in ['Soma', 'Dend']:
    for range_idx in [0,1,2]:
        ...
        globals()[f'attr1_dend_std_root_v_{range_idx}_area'], globals()[f'attr2_dend_std_root_v_{range_idx}_area'] = full_analyze_case(f'clus_invivo_variedW_tau43_addNaK_multiclus_woAP+Ca_aligned_varyinh', 
                                                                                                                                    f'distr_invivo_variedW_tau43_addNaK_multiclus_woAP+Ca_aligned_varyinh', 
    ...
    for range_idx in [0,1,2]:
        calculate_ratio_and_draw_heatmap(globals()[f'attr1_dend_matrix_{range_idx}_area'], globals()[f'attr2_dend_matrix_{range_idx}_area'], 
                                         ...
                                         f'(Invivo {anal_loc} {rec_loc} Range {range_idx} {rec_attr})',
                                         attr1, attr2)
