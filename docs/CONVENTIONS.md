...
| `--num_syn_basal_exc` | `NUM_SYN_BASAL_EXC` | `initialize_synapse_layout()` |
| `--bg_exc_freq` | `bg_exc_freq` → `self.FREQ_EXC` | `add_background_exc_inputs()` |
| `--initW` | `initW` → `self.initW` | `_exc_init_w_distr_array()` |
| `--bg_syn_pos_seed` | `WorkflowSeeds.bg_syn_pos` | Synapse placement and invivo excitatory weights via `random_streams` |
| `--clus_syn_pos_seed` | `WorkflowSeeds.clus_syn_pos` | Cluster assignment, preunit activation order, and invitro clustered weights |
| `--bg_spike_gen_seed` | `WorkflowSeeds.bg_spike_gen` | Pink noise and background excitatory/inhibitory spike streams |
| `--clus_spike_gen_seed` | `WorkflowSeeds.clus_spike_gen` | Clustered presynaptic timing generated by `generate_vecstim()` |
| `--syn_pos_seed` | Deprecated fallback | Used only when either explicit position seed is absent |
...
| `--max_workers_synapse` | `max_workers_synapse` | `L5PNModel(max_workers_synapse=...)` → thread pools |
