...
from utils.random_streams import background_inhibitory_spike_rng, pink_noise_rng

def make_noise(num_traces=10, num_samples=1000, spike_gen_seed=42, scale=0.5):
    noise_rnd = pink_noise_rng(spike_gen_seed)
    ...
    for i in np.arange(0, num_traces):
        # Create White Noise
        wn = noise_rnd.normal(loc=1, scale=scale, size=num_samples)  # scale=0.5
        # Create '1/f' Noise
...
def visualize_pink_noise(DURATION=1000):
    ...
    for scale in [20]:
        ...
        spk_rnd = background_inhibitory_spike_rng(42)
