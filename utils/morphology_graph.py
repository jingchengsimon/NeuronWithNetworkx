...
def create_directed_graph(all_sections, all_segments, section_df, return_segment_graph=False):
    """
    Build the section-level directed graph.

    When return_segment_graph=True, also return a segment-level graph where
    segments inside each section are connected in series and child sections are
    connected to the closest parent segment.
    """
    ...
    segment_df, segment_DiG = create_segment_directed_graph(all_sections, all_segments, section_df)
    ...
    if return_segment_graph:
        return section_df, DiG, segment_df, segment_DiG

    return section_df, DiG

def create_segment_directed_graph(all_sections, all_segments, section_df=None):
    """
    Build a directed graph at segment resolution.

    Nodes are segment ids in the order of all_segments. Edges represent cable
    continuity: consecutive segments inside one section are connected in series,
    and the parent section's connected segment points to the child section's first
    segment.
    """
    segment_rows = []
    segment_DiG = nx.DiGraph()
    segment_key_to_id = {}
    segment_obj_to_id = {id(segment): idx for idx, segment in enumerate(all_segments)}
    section_name_to_id = {}

    if section_df is not None and not section_df.empty:
        section_name_to_id = {
            row['section_name']: int(row['section_id'])
    ...
    for section_id, segment_list in enumerate(all_sections):
        ...
        section = segment_list[0].sec
        section_name = section.psection()['name']
        section_id = section_name_to_id.get(section_name, section_id)
        section_type = _section_type_from_name(section_name)
        section_length = float(section.psection()['morphology']['L'])
        segment_length = section_length / len(segment_list)

        for segment_idx, segment in enumerate(segment_list):
            segment_id = segment_obj_to_id[id(segment)]
            segment_key_to_id[_segment_key(segment)] = segment_id
            ...
            segment_DiG.add_node(segment_id, **attrs)
            segment_rows.append({'segment_id': segment_id, **attrs})

            if segment_idx > 0:
                prev_segment_id = segment_obj_to_id[id(segment_list[segment_idx - 1])]
                segment_DiG.add_edge(
                    prev_segment_id,
                    segment_id,
                    length=segment_length,
    ...
    for section_id, segment_list in enumerate(all_sections):
        if section_id == 0 or not segment_list:
            continue

        section = segment_list[0].sec
        parent_segment = section.parentseg()
        parent_segment_id = _find_connected_parent_segment_id(
            parent_segment,
            segment_key_to_id,
            all_sections,
            all_segments,
        )
        child_segment_id = segment_obj_to_id[id(segment_list[0])]

        if parent_segment_id is None:
            continue

        child_length = segment_DiG.nodes[child_segment_id].get('length')
        segment_DiG.add_edge(
            parent_segment_id,
            child_segment_id,
            length=child_length,
    ...
    remove_cycles(segment_DiG)
    return pd.DataFrame(segment_rows), segment_DiG

def _section_type_from_name(section_name):
    match = re.search(r'\.(.*?)\[', section_name)
    if match:
        return match.group(1)
    return section_name.split('[')[0].split('.')[-1]

def _segment_key(segment):
    return (segment.sec.psection()['name'], round(float(segment.x), 12))

def _find_connected_parent_segment_id(parent_segment, segment_key_to_id, all_sections, all_segments):
    parent_key = _segment_key(parent_segment)
    if parent_key in segment_key_to_id:
        return segment_key_to_id[parent_key]

    parent_section = parent_segment.sec
    parent_section_name = parent_section.psection()['name']
    parent_x = float(parent_segment.x)
    segment_obj_to_id = {id(segment): idx for idx, segment in enumerate(all_segments)}

    for segment_list in all_sections:
        if not segment_list or segment_list[0].sec != parent_section:
            continue
        nearest_segment = min(segment_list, key=lambda segment: abs(float(segment.x) - parent_x))
        return segment_obj_to_id[id(nearest_segment)]
...
def nx_to_clade(graph, node, visited=None):
    ...
    if Clade is None:
        raise ImportError("BioPython is required to convert a NetworkX graph to a Phylo clade.")
