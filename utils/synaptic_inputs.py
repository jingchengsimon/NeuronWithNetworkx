...
import json
import ast
from utils.synapse_models import AMPANMDA
from utils.pink_noise import make_noise
from utils.random_streams import (
    background_exc_synapse_rng,
    background_inhibitory_spike_rng,
    excitatory_weight_rng,
)
...
def _exc_init_w_distr_array(num_syn, initW, syn_pos_seed, use_fixedW, fixedW, *, replace=True):
    ...
    loc_rnd = excitatory_weight_rng(syn_pos_seed)
    sigma = 1
...
def add_background_exc_inputs(section_synapse_df, syn_param_exc, DURATION, FREQ_EXC,
                              ...
                              max_workers_synapse, use_fixedW=False, fixedW=0.0004):
    ...
    def process_section(i):
        ...
        # Independent deterministic RNG per synapse keeps worker scheduling irrelevant.
        spk_rnd = background_exc_synapse_rng(spike_gen_seed, i)
        ...
        if section['synapse'] is None:
            ...
            if bg_exc_channel_type == 'Exp2Syn':
                ...
                syn_params = json.load(open('./model/AMPANMDA.json', 'r'))
        ...
        if spat_condition == 'clus' and num_clus_condition == 'single':
            # Use the deterministic per-synapse stream to generate spike counts independently.
        ...
        if section['netcon'] is not None:
            try:
                ...
                pass  # stale HocObject ref after repeated h.run(); safe to skip
...
def add_background_inh_inputs(section_synapse_df, syn_param_inh, DURATION, FREQ_INH,
    ...
    spk_rnd = background_inhibitory_spike_rng(spike_gen_seed)
    ...
    total_spikes = np.zeros(DURATION)
